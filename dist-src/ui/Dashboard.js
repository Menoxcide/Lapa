    ...
    return (<div className="dashboard p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="lg:col-span-2">
          <LiveGraph nodes={state.nodes} edges={state.edges} onNodeClick={(nodeId) => console.log(`Clicked node: ${nodeId}`)}/>
        </div>
        <div>
          <ControlPanel isRunning={state.isRunning} onPause={pauseSwarm} onResume={resumeSwarm} onRedirect={redirectTask} onReset={resetSwarm}/>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <AgentAvatars agents={state.agents} onAgentClick={(agentId) => console.log(`Clicked agent: ${agentId}`)}/>
        <SpeechBubbles messages={state.messages} onMessageClick={(messageId) => console.log(`Clicked message: ${messageId}`)}/>
      </div>
