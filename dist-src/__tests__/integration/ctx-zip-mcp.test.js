import { compressContext, decompressContext, storeCompressedContext, loadCompressedContext, testCtxZipCompression, recordCompressionStats, recordCompressionFeedback, analyzeCompressionEffectiveness, CtxZipFeedbackController } from '../../src/mcp/ctx-zip.integration';
...
describe('ctx-zip and MCP Integration', () => {
    describe('Context Compression in Handoff Workflow', () => {
        ...
        it('should compress context during handoff with high priority', async () => {
            const largeContext = {
            ...
            expect(receivedContext).toEqual(largeContext);
        });
        it('should maintain semantic meaning after compression/decompression', async () => {
            ...
            expect(receivedContext.userStory).toBe(semanticContext.userStory);
            expect(receivedContext.acceptanceCriteria).toEqual(semanticContext.acceptanceCriteria);
            expect(receivedContext.technicalRequirements).toEqual(semanticContext.technicalRequirements);
        });
        it('should handle different compression priorities appropriately', async () => {
            const testContext = {
            ...
            expect(lowPriorityResponse.success).toBe(true);
            ...
            expect(lowPrioritySize).toBeGreaterThan(0);
            expect(highPrioritySize).toBeGreaterThan(0);
        });
    });
    describe('ctx-zip Effectiveness Validation', () => {
        it('should achieve >80% compression reduction for large payloads', async () => {
      ...
      `.repeat(1000); // Make it large enough to test effectiveness
            ...
            expect(stats.compressionRatio).toBeGreaterThan(5); // At least 5x compression
        ...
        it('should maintain effectiveness across different content types', async () => {
            const contentTypes = [
                {
                    name: 'Source Code',
          ...
          `.repeat(50)
            ...
            for (const { name, content } of contentTypes) {
                const stats = await testCtxZipCompression(content);
                ...
                expect(stats.reductionPercentage).toBeGreaterThan(70); // At least 70% for any content
        ...
        it('should track and analyze compression statistics', async () => {
            ...
            for (let i = 0; i < testPayloads.length; i++) {
                ...
                await recordCompressionStats({
                    ...stats,
                    sessionId: `test-session-${i}`
                });
            }
            // Record some feedback
            await recordCompressionFeedback({
    ...
    describe('Feedback Loop Integration', () => {
        it('should process feedback and provide optimization suggestions', async () => {
            const feedbackController = new CtxZipFeedbackController();
            ...
            expect(feedbackResult.avgEffectiveness).toBeGreaterThan(0);
            expect(feedbackResult.avgSemanticPreservation).toBeGreaterThan(0);
            expect(feedbackResult.compressionImprovement).toBeDefined();
            ...
            expect(combinedResult.avgEffectiveness).toBe(0);
            expect(combinedResult.avgSemanticPreservation).toBe(0);
            expect(combinedResult.compressionImprovement).toBe('No feedback data available');
        });
    });
    describe('Storage Integration', () => {
        it('should store and load compressed contexts correctly', async () => {
            ...
            const loadedCompressed = await loadCompressedContext(sessionId);
            // Verify the compressed data is the same
            expect(loadedCompressed).toEqual(compressed);
            // Decompress to verify integrity
            const decompressed = await decompressContext(loadedCompressed);
            expect(decompressed).toBe(testContext);
        });
        it('should handle storage of multiple contexts', async () => {
            const contexts = [
            ...
            for (const ctx of contexts) {
                const compressed = await compressContext(ctx.data);
                await storeCompressedContext(ctx.id, compressed);
            }
            // Load and verify all contexts
            for (const ctx of contexts) {
                const loadedCompressed = await loadCompressedContext(ctx.id);
                const decompressed = await decompressContext(loadedCompressed);
                expect(decompressed).toBe(ctx.data);
            }
        });
    });
    describe('Performance Under Load', () => {
        it('should maintain compression quality with repeated operations', async () => {
            ...
            expect(maxReduction).toBeLessThan(95); // Should not be perfect compression
