...
describe('HybridHandoffSystem', () => {
    ...
    describe('Handoff Evaluation', () => {
        it('should evaluate handoff with OpenAI agent when enabled', async () => {
            ...
            const evaluation = await handoffSystem.evaluateHandoff(testContext);
        ...
        it('should handle evaluation errors gracefully', async () => {
            ...
            run.mockRejectedValue(new Error('API timeout'));
            const testContext = {
                task: { id: 'task-123', description: 'Test task' },
                context: {}
            };
    ...
    describe('Handoff Initiation with OpenAI Agents', () => {
        ...
        it('should fail handoff after max retries exceeded', async () => {
            ...
            // Always fail
            run.mockRejectedValue(new Error('Persistent error'));
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'Test OpenAI Agent', 'task-456', { testData: 'sample context data' })).rejects.toThrow('Failed to handoff to OpenAI agent: Persistent error');
    ...
    describe('Lifecycle Hooks', () => {
        it('should call lifecycle hooks during handoff process', async () => {
            ...
            expect(hooks.onHandoffStart).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'task-456');
            expect(hooks.onHandoffComplete).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'task-456', expect.any(Number) // Duration
            );
        ...
        it('should call error hook when handoff fails', async () => {
            ...
            handoffSystem.registerOpenAIAgent(mockOpenAIAgent);
            // Mock an error
            run.mockRejectedValue(new Error('Handoff failed'));
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'Test OpenAI Agent', 'task-456', { testData: 'sample context data' })).rejects.toThrow('Failed to handoff to OpenAI agent: Handoff failed');
            expect(hooks.onHandoffStart).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'task-456');
            expect(hooks.onHandoffError).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'task-456', expect.any(Error));
    ...
    describe('Performance Monitoring', () => {
        ...
        it('should meet latency target for fast handoffs', async () => {
            ...
            const consoleWarnSpy = jest.spyOn(console, 'warn').mockImplementation();
            await handoffSystem.initiateHandoff('source-agent-123', 'Test OpenAI Agent', 'task-456', { testData: 'sample context data' });
            expect(consoleWarnSpy).not.toHaveBeenCalledWith(expect.stringContaining('Handoff latency target exceeded'));
            consoleWarnSpy.mockRestore();
        });
    });
