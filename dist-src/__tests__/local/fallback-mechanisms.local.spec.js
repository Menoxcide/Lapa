...
describe('Fallback Mechanisms Tests', () => {
    ...
    describe('Multiple Fallback Attempts', () => {
        ...
        it('should gracefully handle when all fallbacks fail', async () => {
            ...
            sendOllamaChatRequest.mockRejectedValue(new Error('All Ollama attempts failed'));
            sendNIMInferenceRequest.mockRejectedValue(new Error('All NIM attempts failed'));
            await expect(handoffSystem.handoffToLocalAgent(mockOllamaAgent, 'all-fail-test-123', { testData: 'context data for all fail test' }, 'source-agent-123', 'ollama-agent-1')).rejects.toThrow('Failed to handoff to local agent');
        }, 10000);
    });
    describe('Performance During Fallback', () => {
        it('should maintain latency targets during fallback operations', async () => {
            ...
            sendNIMInferenceRequest.mockImplementation(async () => {
        ...
        it('should log appropriate warnings when fallback occurs', async () => {
            ...
            isNIMAvailable.mockResolvedValue(true);
            // Mock NIM response
            sendNIMInferenceRequest.mockResolvedValue('Task completed with fallback');
            // Spy on console.warn
