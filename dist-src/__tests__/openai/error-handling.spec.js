...
describe('OpenAI Handoff Error Handling', () => {
    ...
    describe('Retry Logic Validation', () => {
        ...
        it('should fail after max retries exceeded with OpenAI errors', async () => {
            ...
            // Always fail
            run.mockRejectedValue(new Error('Persistent OpenAI API error'));
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'Test OpenAI Agent', 'fail-task-456', { testData: 'context data that always fails' })).rejects.toThrow('Failed to handoff to OpenAI agent: Persistent OpenAI API error');
    ...
    describe('Failure Recovery Scenarios', () => {
        ...
        it('should handle OpenAI agent not found errors', async () => {
            handoffSystem.registerOpenAIAgent(mockOpenAIAgent);
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'NonExistent OpenAI Agent', 'not-found-task-456', { testData: 'context data for non-existent agent' })).rejects.toThrow('Failed to handoff to OpenAI agent: OpenAI agent NonExistent OpenAI Agent not found');
        });
    });
    describe('Error Propagation and Logging', () => {
        it('should propagate OpenAI API errors with context information', async () => {
            ...
            run.mockRejectedValue(new Error('Invalid API key provided'));
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'Test OpenAI Agent', 'api-error-task-456', { testData: 'context data with API error' })).rejects.toThrow('Failed to handoff to OpenAI agent: Invalid API key provided');
        ...
        it('should log errors appropriately during OpenAI handoff failures', async () => {
            ...
            // Mock API error
            run.mockRejectedValue(new Error('OpenAI service overloaded'));
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'Test OpenAI Agent', 'logging-error-task-456', { testData: 'context data with logging' })).rejects.toThrow('Failed to handoff to OpenAI agent: OpenAI service overloaded');
            // Check that error was logged
            expect(consoleErrorSpy).toHaveBeenCalledWith('Handoff to OpenAI agent Test OpenAI Agent failed:', expect.any(Error));
            consoleErrorSpy.mockRestore();
        });
        it('should handle non-Error objects thrown by OpenAI SDK', async () => {
            handoffSystem.registerOpenAIAgent(mockOpenAIAgent);
            // Mock throwing a string instead of an Error object
            run.mockRejectedValue('String error from OpenAI SDK');
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'Test OpenAI Agent', 'string-error-task-456', { testData: 'context data with string error' })).rejects.toThrow('Failed to handoff to OpenAI agent: String error from OpenAI SDK');
            expect(run).toHaveBeenCalledTimes(1);
        });
    });
    describe('Edge Case Error Handling', () => {
        it('should handle retry configuration edge cases', async () => {
            ...
            run.mockRejectedValue(new Error('Immediate failure with zero retries'));
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'Test OpenAI Agent', 'zero-retry-task-456', { testData: 'context data with zero retries' })).rejects.toThrow('Failed to handoff to OpenAI agent: Immediate failure with zero retries');
