...
describe('Hybrid Handoff System Integration', () => {
    ...
    describe('End-to-End Task Execution', () => {
        it('should execute task with OpenAI agent handoff recommendation', async () => {
            ...
            expect(run).toHaveBeenNthCalledWith(1, mockOpenAIAgent, expect.stringContaining('Evaluate this context for handoff'));
            expect(run).toHaveBeenNthCalledWith(2, mockOpenAIAgent, expect.stringContaining('Process this task:'));
        }, 15000);
        it('should execute task without handoff when not recommended', async () => {
            ...
            run.mockResolvedValueOnce(mockEvaluationResult);
            const task = {
                id: 'simple-task-123',
