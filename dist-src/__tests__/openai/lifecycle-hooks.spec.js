...
describe('OpenAI Handoff Lifecycle Hooks', () => {
    ...
    describe('Hook Registration and Execution', () => {
        it('should execute onHandoffStart hook when OpenAI handoff begins', async () => {
            ...
            run.mockResolvedValue(mockRunResult);
            ...
            expect(hooks.onHandoffStart).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'hook-test-task-456');
            // Complete hook should also be called
            expect(hooks.onHandoffComplete).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'hook-test-task-456', expect.any(Number) // Duration
            );
        ...
        it('should execute onHandoffComplete hook with accurate timing information', async () => {
            ...
            expect(hooks.onHandoffComplete).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'timing-test-task-456', expect.closeTo(actualDuration, 50) // Allow 50ms tolerance
            );
        });
        it('should execute onHandoffError hook when OpenAI handoff fails', async () => {
            ...
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'Test OpenAI Agent', 'error-hook-test-task-456', { testData: 'context data for error hook test' })).rejects.toThrow(`Failed to handoff to OpenAI agent: ${errorMessage}`);
            // Start hook should be called
            expect(hooks.onHandoffStart).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'error-hook-test-task-456');
            // Error hook should be called with proper error
            expect(hooks.onHandoffError).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'error-hook-test-task-456', expect.any(Error));
    ...
    describe('Hook Error Handling', () => {
        it('should continue OpenAI handoff execution even if onHandoffStart hook throws', async () => {
            ...
            expect(hooks.onHandoffStart).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'hook-error-test-task-456');
            // Complete hook should still be called
            expect(hooks.onHandoffComplete).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'hook-error-test-task-456', expect.any(Number));
            // Error hook should not be called for hook errors
            expect(hooks.onHandoffError).not.toHaveBeenCalled();
            // Hook error should be logged
            expect(consoleErrorSpy).toHaveBeenCalledWith('Error in onHandoffStart hook:', expect.any(Error));
        ...
        it('should continue OpenAI handoff execution even if onHandoffComplete hook throws', async () => {
            ...
            // Start hook should be called
            expect(hooks.onHandoffStart).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'complete-hook-error-test-task-456');
            // Complete hook should have been called
            expect(hooks.onHandoffComplete).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'complete-hook-error-test-task-456', expect.any(Number));
            // Error hook should not be called for hook errors
            expect(hooks.onHandoffError).not.toHaveBeenCalled();
            // Hook error should be logged
            expect(consoleErrorSpy).toHaveBeenCalledWith('Error in onHandoffComplete hook:', expect.any(Error));
            consoleErrorSpy.mockRestore();
        });
        it('should handle errors in onHandoffError hook without affecting error reporting', async () => {
            ...
            run.mockRejectedValue(new Error(errorMessage));
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'Test OpenAI Agent', 'error-handler-error-test-task-456', { testData: 'context data for error handler error test' })).rejects.toThrow(`Failed to handoff to OpenAI agent: ${errorMessage}`);
            // Start hook should be called
            expect(hooks.onHandoffStart).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'error-handler-error-test-task-456');
            // Error hook should have been called
            expect(hooks.onHandoffError).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'error-handler-error-test-task-456', expect.any(Error));
            // Complete hook should not be called
            expect(hooks.onHandoffComplete).not.toHaveBeenCalled();
            // Hook error should be logged
            expect(consoleErrorSpy).toHaveBeenCalledWith('Error in onHandoffError hook:', expect.any(Error));
            consoleErrorSpy.mockRestore();
        });
    });
    describe('Hook Timing and Order', () => {
        ...
        it('should execute hooks in correct order during failed OpenAI handoff', async () => {
            ...
            handoffSystem = new HybridHandoffSystem({}, hooks);
            handoffSystem.registerOpenAIAgent(mockOpenAIAgent);
            run.mockRejectedValue(new Error('OpenAI API error'));
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'Test OpenAI Agent', 'error-order-test-task-456', { testData: 'context data for error order test' })).rejects.toThrow('Failed to handoff to OpenAI agent: OpenAI API error');
        ...
        it('should execute hooks with correct parameters for complex handoff scenarios', async () => {
            ...
            expect(hooks.onHandoffStart).toHaveBeenCalledWith('agent-1', 'Test OpenAI Agent', 'complex-task-789');
            // Verify complete hook parameters
            expect(hooks.onHandoffComplete).toHaveBeenCalledWith('agent-1', 'Test OpenAI Agent', 'complex-task-789', expect.any(Number) // Duration
            );
            // Error hook should not be called
            expect(hooks.onHandoffError).not.toHaveBeenCalled();
        });
    ...
    describe('Hook Integration with Retry Logic', () => {
        it('should execute hooks appropriately during OpenAI handoff retries', async () => {
            ...
            expect(hooks.onHandoffStart).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'retry-hook-test-task-456');
            ...
            expect(hooks.onHandoffComplete).toHaveBeenCalledWith('source-agent-123', 'Test OpenAI Agent', 'retry-hook-test-task-456', expect.any(Number));
        });
        it('should execute error hook for each failed OpenAI attempt', async () => {
            ...
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'Test OpenAI Agent', 'all-fail-hook-test-task-456', { testData: 'context data for all fail hook test' })).rejects.toThrow('Failed to handoff to OpenAI agent: Attempt 3: Service unavailable');
            // Start hook should be called once
            expect(hooks.onHandoffStart).toHaveBeenCalledTimes(1);
