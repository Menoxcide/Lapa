...
describe('OpenAI Agent Integration', () => {
    ...
    describe('OpenAI Agent Execution', () => {
        it('should execute task on OpenAI agent successfully', async () => {
            ...
            run.mockResolvedValue(mockExecutionResult);
            ...
            expect(run).toHaveBeenCalledWith(mockOpenAIAgent, expect.stringContaining('Process this task:'));
        });
        ...
        it('should reject when OpenAI agent execution throws error', async () => {
            handoffSystem.registerOpenAIAgent(mockOpenAIAgent);
            run.mockRejectedValue(new Error('OpenAI API rate limit exceeded'));
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'Test OpenAI Agent', 'task-456', { testData: 'sample context data' })).rejects.toThrow('Failed to handoff to OpenAI agent: OpenAI API rate limit exceeded');
    ...
    describe('Edge Cases and Error Handling', () => {
        ...
        it('should handle execution when target OpenAI agent is not found', async () => {
            ...
            await expect(handoffSystem.initiateHandoff('source-agent-123', 'NonExistent OpenAI Agent', 'task-456', { testData: 'sample context data' })).rejects.toThrow('Failed to handoff to OpenAI agent: OpenAI agent NonExistent OpenAI Agent not found');
        });
        it('should handle evaluation when OpenAI agent throws error', async () => {
            handoffSystem.registerOpenAIAgent(mockOpenAIAgent);
            run.mockRejectedValue(new Error('Network connectivity issue'));
            const testContext = {
                task: { id: 'task-123', description: 'Test task' },
                context: {}
            };
