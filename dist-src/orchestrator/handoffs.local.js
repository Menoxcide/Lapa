 ...
 */
import { LangGraphOrchestrator } from '../swarm/langgraph.orchestrator';
import { ContextHandoffManager } from '../swarm/context.handoff';
import { moeRouter } from '../agents/moe-router';
 ...
 */
export class LocalHandoffSystem {
    ...
    async evaluateHandoff(context, task) {
        ...
        try {
            ...
            if (evaluatorAgent.type === 'ollama') {
                evaluationResult = await sendOllamaChatRequest(evaluatorAgent.model, [{ role: 'user', content: `Evaluate this context and task for handoff: ${evaluationTask.input}` }]);
            }
            else if (evaluatorAgent.type === 'nim') {
                evaluationResult = await sendNIMInferenceRequest(evaluatorAgent.model, `Evaluate this context and task for handoff: ${evaluationTask.input}`);
            }
     ...
     */
    async handoffToLocalAgent(targetAgent, taskId, context, sourceAgentId, targetAgentId) {
        try {
            const handoffTask = {
                id: taskId,
            ...
            if (targetAgent.type === 'ollama') {
                result = await this.runWithRetry(() => sendOllamaChatRequest(targetAgent.model, [{ role: 'user', content: `Process this task: ${handoffTask.input}` }]), `Handoff to Ollama agent ${targetAgentId}`);
            }
            else if (targetAgent.type === 'nim') {
                result = await this.runWithRetry(() => sendNIMInferenceRequest(targetAgent.model, `Process this task: ${handoffTask.input}`), `Handoff to NIM agent ${targetAgentId}`);
            }
            ...
            if (targetAgent.type === 'ollama' && await isNIMAvailable()) {
                ...
                try {
                    ...
                    const nimResult = await this.runWithRetry(() => sendNIMInferenceRequest(targetAgent.model, `Process this task: ${JSON.stringify(context)}`), `Fallback to NIM agent ${targetAgentId}`);
                ...
                try {
                    // Use the AI SDK to make the request
                    const ollamaResult = await this.runWithRetry(() => sendOllamaChatRequest(targetAgent.model, [{ role: 'user', content: `Process this task: ${JSON.stringify(context)}` }]), `Fallback to Ollama agent ${targetAgentId}`);
    ...
    async localHandoff(task, context) {
        try {
            ...
            // Execute handoff
            const result = await this.handoffToLocalAgent(targetAgent, task.id, context, 'current-agent', // source agent
            targetAgentId);
...
export const localHandoffSystem = new LocalHandoffSystem();
/**
 ...
 */
export function createOpenAI(baseURL = 'http://localhost:8000/v1', apiKey) {
 ...
 */
export function createOllama(baseURL = 'http://localhost:11434/api') {
 ...
 * @returns Promise that resolves with the handoff result
 */
export async function localHandoff(task, context, preferredProvider = 'ollama') {
    const startTime = performance.now();
    try {
