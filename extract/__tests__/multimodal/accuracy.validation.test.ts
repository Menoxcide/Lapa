  ...
  describe('Accuracy Validation Reporting', () => {
    it('should generate comprehensive accuracy validation report', async () => {
      ...
      for (const result of results) {
        ...
        const expectedMin = typeof result.expectedValue === 'number' 
          ? result.expectedValue 
          : (result.expectedValue as { min: number; max?: number }).min;
        expect(result.actualValue).toBeGreaterThanOrEqual(expectedMin);
      }
    });
