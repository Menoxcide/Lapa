...
// AutoGen-specific llama.cpp configuration
interface AutoGenLlamaCppConfig {
...
const DEFAULT_CONFIG: AutoGenLlamaCppConfig = {
...
interface LlamaCppIntegrationStatus {
  isAvailable: boolean;
  isExecutableFound: boolean;
  isModelFound: boolean;
 ...
 */
export class AutoGenLlamaCppAdapter {
  private config: AutoGenLlamaCppConfig;
  private status: LlamaCppIntegrationStatus;

  constructor(config?: Partial<AutoGenLlamaCppConfig>) {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.status.isExecutableFound = existsSync(this.config.executablePath);
      if (!this.status.isExecutableFound) {
        throw new Error(`llama.cpp executable not found at ${this.config.executablePath}`);
      }
      
      // Check if model file exists
      this.status.isModelFound = existsSync(this.config.defaultModelPath);
  ...
  private async executeLlamaCpp(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (parameters.temp !== undefined) args.push('--temp', parameters.temp.toString());
      if (parameters.top_k !== undefined) args.push('--top-k', parameters.top_k.toString());
      if (parameters.top_p !== undefined) args.push('--top-p', parameters.top_p.toString());
      if (parameters.repeat_penalty !== undefined) args.push('--repeat-penalty', parameters.repeat_penalty.toString());
      ...
      const child = spawn(this.config.executablePath, args, { stdio: ['pipe', 'pipe', 'pipe'] });
   ...
   */
  getStatus(): LlamaCppIntegrationStatus {
    return { ...this.status };
  }
   ...
   */
  updateConfig(newConfig: Partial<AutoGenLlamaCppConfig>): void {
...
export const autoGenLlamaCppAdapter = new AutoGenLlamaCppAdapter();

// Export types for external use
export type { AutoGenLlamaCppConfig, LlamaCppIntegrationStatus };