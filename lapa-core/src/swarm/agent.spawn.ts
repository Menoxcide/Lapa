 ...
 */
export class AgentSpawningSystem {
  private agentTemplates: Map<AgentType, AgentTemplate> = new Map();
  private spawnedAgents: Map<string, Agent> = new Map();
  private spawnCounter: Map<AgentType, number> = new Map();
   ...
   */
  private async summarizeContext(context: Record<string, any>, options: SummaryOptions): Promise<string> {
    try {
      ...
      try {
        const summary = await sendNIMInferenceRequest(
          'llama3.1', // Using llama3.1 as default model
          prompt,
          {
   ...
   */
  private createSummaryPrompt(contextString: string, options: SummaryOptions): string {
...
`.trim();
   ...
   */
  private fallbackSummarize(contextString: string, maxLength: number): string {
   ...
   */
  private extractFocusAreas(task: Task, template: AgentTemplate): string[] {
    const focusAreas: string[] = [...template.expertise];
...
export const agentSpawningSystem = new AgentSpawningSystem();