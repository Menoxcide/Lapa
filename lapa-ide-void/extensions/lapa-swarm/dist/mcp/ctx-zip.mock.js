...
function mockCompressData(data) {
...
async function compress(context) {
    ...
    const keyBuffer = buffer_1.Buffer.from(key);
    const compressedBuffer = buffer_1.Buffer.from(compressed);
    const result = buffer_1.Buffer.concat([
        buffer_1.Buffer.from(keyBuffer.length.toString().padStart(4, '0')),
        keyBuffer,
        compressedBuffer
    ]);
    return result;
...
async function decompress(compressedContext) {
