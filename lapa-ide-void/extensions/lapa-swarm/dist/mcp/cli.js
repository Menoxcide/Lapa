...
exports.showToolInfo = showToolInfo;
exports.showHelp = showHelp;
const scaffolding_ts_1 = require("./scaffolding.ts");
 ...
 */
async function generateTool(args) {
    if (!args.toolName) {
    ...
    try {
        if (args.inputSchema.endsWith('.json')) {
            // Read from file
            const schemaContent = await (0, promises_1.readFile)(args.inputSchema, 'utf-8');
    ...
    if (args.outputSchema) {
        try {
            if (args.outputSchema.endsWith('.json')) {
                // Read from file
                const schemaContent = await (0, promises_1.readFile)(args.outputSchema, 'utf-8');
    ...
    try {
        // Generate the tool
        const metadata = await scaffolding_ts_1.mcpScaffolding.generateTool(config);
...
async function listTools() {
    const tools = scaffolding_ts_1.mcpScaffolding.getGeneratedTools();
    if (tools.length === 0) {
 ...
 * @param args CLI arguments
 */
async function showToolInfo(args) {
    if (!args.toolName) {
        console.error('Error: Tool name is required');
        process.exit(1);
    }
    const metadata = scaffolding_ts_1.mcpScaffolding.getToolMetadata(args.toolName);
    if (!metadata) {
