...
exports.MCPConnector = exports.MCP_PROTOCOL_VERSION = void 0;
exports.createMCPConnector = createMCPConnector;
...
class MCPConnector {
    ...
    jsonSchemaToZod(schema) {
        ...
        if (schema.oneOf) {
            const schemas = schema.oneOf.map((subSchema) => this.jsonSchemaToZod(subSchema));
            if (schemas.length === 1) {
                return schemas[0];
            }
            return zod_1.z.union(schemas);
        }
        if (schema.anyOf) {
            const schemas = schema.anyOf.map((subSchema) => this.jsonSchemaToZod(subSchema));
            if (schemas.length === 1) {
                return schemas[0];
            }
            return zod_1.z.union(schemas);
        ...
        if (schema.enum) {
            if (schema.enum.length === 1) {
                return zod_1.z.literal(schema.enum[0]);
            }
        ...
        switch (schema.type) {
            case 'object':
                ...
                const objSchema = zod_1.z.object(shape);
            ...
            case 'array':
                ...
                let arrSchema = zod_1.z.array(itemSchema);
            ...
            case 'number':
            case 'integer':
                let numSchema = schema.type === 'integer' ? zod_1.z.number().int() : zod_1.z.number();
...
function createMCPConnector(config) {
