...
async function isOllamaAvailable() {
    try {
        const ollama = new ollama_1.Ollama({ host: OLLAMA_HOST });
        await ollama.list();
        return true;
    }
    catch (error) {
 ...
 */
async function sendOllamaInferenceRequest(model, prompt, parameters = {}) {
    try {
        const ollama = new ollama_1.Ollama({ host: OLLAMA_HOST });
        const requestBody = {
            model,
 ...
 * @returns Model response
 */
async function sendOllamaChatRequest(model, messages, parameters = {}) {
    try {
        const ollama = new ollama_1.Ollama({ host: OLLAMA_HOST });
        const requestBody = {
            model,
 ...
 */
async function initializeOllamaEnvironment() {
    try {
