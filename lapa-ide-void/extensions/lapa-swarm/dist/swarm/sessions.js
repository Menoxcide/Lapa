...
exports.swarmSessionManager = exports.SwarmSessionManager = void 0;
exports.createSwarmSession = createSwarmSession;
exports.generateSessionShareLink = generateSessionShareLink;
exports.getSessionShareInfo = getSessionShareInfo;
exports.joinSwarmSession = joinSwarmSession;
exports.requestVeto = requestVeto;
exports.initiateSessionA2AHandshake = initiateSessionA2AHandshake;
...
const sessionConfigSchema = zod_1.z.object({
    ...
    maxParticipants: zod_1.z.number().min(2).max(50),
    enableVetoes: zod_1.z.boolean(),
    enableA2A: zod_1.z.boolean(),
    webrtcConfig: zod_1.z.object({
        ...
        iceTransportPolicy: zod_1.z.enum(['all', 'relay']).optional(),
        iceCandidatePoolSize: zod_1.z.number().optional()
    }).optional(),
    signalingConfig: zod_1.z.object({
        serverUrl: zod_1.z.string().optional(),
        enableSignaling: zod_1.z.boolean().optional(),
        fallbackToDirect: zod_1.z.boolean().optional()
    }).optional()
});
...
class SignalingClient {
    ...
    sessionId;
    signalingServerUrl;
    constructor(signalingServerUrl, participantId, sessionId) {
...
class WebRTCConnectionManager {
    ...
    useSignalingServer = false;
    signalingServerUrl = '';
    participantId = '';
    sessionId = '';
    constructor(iceServers, signalingServerUrl) {
     ...
     */
    createPeerConnection(config, sessionId, fromUserId, toUserId) {
        const pcConfig = {
     ...
     */
    async initializeSignaling(participantId, sessionId) {
     ...
     */
    async handleSDPOfferViaSignaling(message) {
     ...
     */
    async handleSDPAnswerViaSignaling(message) {
     ...
     */
    async handleICECandidateViaSignaling(message) {
     ...
     */
    async sendSDPOfferViaSignaling(toUserId, connectionId, offer, sessionId) {
        if (this.useSignalingServer && this.signalingClient && this.signalingClient.getIsConnected()) {
     ...
     */
    async sendSDPAnswerViaSignaling(toUserId, connectionId, answer, sessionId) {
        if (this.useSignalingServer && this.signalingClient && this.signalingClient.getIsConnected()) {
...
class SwarmSessionManager {
    ...
    webrtcManager;
    consensusVoting;
    participantSessions = new Map(); // userId -> sessionId
    ...
    constructor(iceServers, signalingServerUrl) {
        ...
        event_bus_ts_1.eventBus.subscribe('webrtc.ice-candidate', async (event) => {
            await this.handleICECandidate(event.payload);
        });
        event_bus_ts_1.eventBus.subscribe('webrtc.connection-state', async (event) => {
            await this.handleConnectionStateChange(event.payload);
        });
        // Subscribe to A2A events for integration
        event_bus_ts_1.eventBus.subscribe('a2a.handshake.response', async (event) => {
            await this.handleA2AHandshakeResponse(event.payload);
        });
        // Subscribe to WebRTC SDP events
        event_bus_ts_1.eventBus.subscribe('webrtc.sdp-offer', async (event) => {
            await this.handleSDPOffer(event.payload);
        });
        event_bus_ts_1.eventBus.subscribe('webrtc.sdp-answer', async (event) => {
            await this.handleSDPAnswer(event.payload);
        });
     ...
     */
    async createSession(config, userId) {
        try {
            // Validate config with Zod
            const validatedConfig = sessionConfigSchema.parse(config);
            // Check RBAC permission for session creation
            const accessCheck = await rbac_ts_1.rbacSystem.checkAccess(userId, validatedConfig.sessionId, 'session', 'session.create');
     ...
     */
    async joinSession(sessionId, userId, displayName, agentId) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            throw new Error(`Session ${sessionId} not found`);
        }
        // Check RBAC permission for joining session
        const accessCheck = await rbac_ts_1.rbacSystem.checkAccess(userId, sessionId, 'session', 'session.join');
        if (!accessCheck.allowed) {
    ...
    async leaveSession(sessionId, userId) {
        ...
        const accessCheck = await rbac_ts_1.rbacSystem.checkAccess(userId, sessionId, 'session', 'session.leave');
        if (!accessCheck.allowed) {
     ...
     */
    async requestVeto(sessionId, taskId, requestedBy, reason) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            throw new Error(`Session ${sessionId} not found`);
        }
        // Check RBAC permission for requesting veto
        const accessCheck = await rbac_ts_1.rbacSystem.checkAccess(requestedBy, taskId, 'task', 'consensus.veto');
        if (!accessCheck.allowed) {
            return {
        ...
        const votingSessionId = this.consensusVoting.createVotingSession(`Veto request for task ${taskId}: ${reason}`, voteOptions, Math.ceil(session.participants.size / 2) // Quorum: majority
        );
     ...
     */
    async initiateA2AHandshake(sessionId, sourceAgentId, targetAgentId, task) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            throw new Error(`Session ${sessionId} not found`);
        }
     ...
     */
    async establishWebRTCConnections(session, newParticipant) {
        ...
        for (const [userId, participant] of session.participants.entries()) {
            ...
            try {
                ...
                await this.webrtcManager.sendSDPOfferViaSignaling(newParticipant.userId, connectionId, offer, session.sessionId);
            ...
            if (newParticipant.peerConnection) {
                try {
                    ...
                    const reversePC = this.webrtcManager.createPeerConnection(session.config.webrtcConfig, session.sessionId, newParticipant.userId, participant.userId);
                    ...
                    // Send offer through signaling server or event bus
                    await this.webrtcManager.sendSDPOfferViaSignaling(participant.userId, reverseConnectionId, offer, session.sessionId);
     ...
     */
    async handleConnectionStateChange(payload) {
        try {
     ...
     */
    async handleA2AHandshakeResponse(response) {
    ...
    async handleSDPOffer(payload) {
        try {
            ...
            await this.webrtcManager.sendSDPAnswerViaSignaling(payload.fromUserId, payload.connectionId, answer, payload.sessionId);
     ...
     */
    async handleTaskMessage(session, message) {
     ...
     */
    async handleVetoMessage(session, message) {
        ...
        const votingSessionId = this.consensusVoting.createVotingSession(`Veto request for task ${payload.taskId}: ${payload.reason}`, voteOptions, Math.ceil(session.participants.size / 2) // Quorum: majority
        );
     ...
     */
    async handleA2AMessage(session, message) {
     ...
     */
    async handleStateMessage(session, message) {
        // Handle state synchronization
        const payload = message.payload;
     ...
     */
    async handleHandoffMessage(session, message) {
...
exports.SwarmSessionManager = SwarmSessionManager;
// Export singleton instance
exports.swarmSessionManager = new SwarmSessionManager();
/**
 * Convenience function for creating a session
 */
async function createSwarmSession(config, userId) {
    ...
    return await exports.swarmSessionManager.createSession(config, userId);
}
/**
 ...
 */
function generateSessionShareLink(sessionId, baseUrl) {
 ...
 */
function getSessionShareInfo(sessionId) {
    const session = exports.swarmSessionManager.getSession(sessionId);
    if (!session) {
        return undefined;
    }
    return {
 ...
 */
async function joinSwarmSession(sessionId, userId, displayName, agentId) {
    ...
    return await exports.swarmSessionManager.joinSession(sessionId, userId, displayName, agentId);
}
/**
 * Convenience function for requesting a veto
 */
async function requestVeto(sessionId, taskId, requestedBy, reason) {
    return await exports.swarmSessionManager.requestVeto(sessionId, taskId, requestedBy, reason);
}
/**
 * Convenience function for initiating A2A handshake in session
 */
async function initiateSessionA2AHandshake(sessionId, sourceAgentId, targetAgentId, task) {
    return await exports.swarmSessionManager.initiateA2AHandshake(sessionId, sourceAgentId, targetAgentId, task);
}
