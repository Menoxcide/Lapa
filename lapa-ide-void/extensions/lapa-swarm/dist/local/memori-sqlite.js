...
exports.autoGenMemoriSQLite = exports.AutoGenMemoriSQLite = void 0;
...
class AutoGenMemoriSQLite {
    ...
    async initialize() {
        try {
            ...
            // Initialize database
            this.db = new sqlite3_1.Database(this.config.dbPath);
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            const dbSerialize = (0, util_1.promisify)(this.db.serialize.bind(this.db));
            // Create tables
            await dbSerialize(() => {
                // Entities table
    ...
    async storeEntity(entity) {
        ...
        try {
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            await dbRun(`INSERT OR REPLACE INTO entities (id, type, value, confidence, timestamp, source_agent_id, task_id)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, entity.id, entity.type, entity.value, entity.confidence, entity.timestamp.toISOString(), entity.sourceAgentId, entity.taskId);
    ...
    async getEntitiesByType(type, limit = 100) {
        ...
        try {
            const dbAll = (0, util_1.promisify)(this.db.all.bind(this.db));
            const rows = await dbAll(`SELECT id, type, value, confidence, timestamp, source_agent_id, task_id
         FROM entities
         WHERE type = ?
    ...
    async storeConversationEntry(entry) {
        ...
        try {
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            await dbRun(`INSERT INTO conversation_history (id, agent_id, task_id, role, content, timestamp)
         VALUES (?, ?, ?, ?, ?, ?)`, entry.id, entry.agentId, entry.taskId, entry.role, entry.content, entry.timestamp.toISOString());
     ...
     */
    async getConversationHistory(taskId, limit = 50) {
        if (!this.isInitialized || !this.db || !this.config.enableConversationHistory) {
            return [];
        }
        try {
            const dbAll = (0, util_1.promisify)(this.db.all.bind(this.db));
            const rows = await dbAll(`SELECT id, agent_id, task_id, role, content, timestamp
    ...
    async storePerformanceMetric(metric) {
        ...
        try {
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            await dbRun(`INSERT INTO performance_metrics (id, agent_id, task_id, metric_type, value, timestamp)
         VALUES (?, ?, ?, ?, ?, ?)`, metric.id, metric.agentId, metric.taskId, metric.metricType, metric.value, metric.timestamp.toISOString());
            // Clean up old entries if we exceed the maximum
     ...
     */
    async getPerformanceMetrics(agentId, metricType, limit = 100) {
        if (!this.isInitialized || !this.db || !this.config.enablePerformanceMetrics) {
            return [];
        }
        try {
     ...
     */
    async cleanupOldEntries(tableName) {
        if (!this.db) {
            return;
        }
        try {
     ...
     */
    async extractEntities(agent, task, result) {
        if (!this.config.enableEntityExtraction) {
            return [];
        }
    ...
    async close() {
        if (this.db) {
            const dbClose = (0, util_1.promisify)(this.db.close.bind(this.db));
            await dbClose();
            this.db = null;
     ...
     */
    async storeSwarmSession(session) {
        if (!this.isInitialized || !this.db) {
            return;
        }
        try {
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            await dbRun(`INSERT OR REPLACE INTO swarm_sessions (session_id, host_user_id, status, created_at, last_activity, config)
         VALUES (?, ?, ?, ?, ?, ?)`, session.sessionId, session.hostUserId, session.status, session.createdAt.toISOString(), session.lastActivity.toISOString(), session.config);
     ...
     */
    async getAllSwarmSessions() {
        if (!this.isInitialized || !this.db) {
            return [];
        }
        try {
            const dbAll = (0, util_1.promisify)(this.db.all.bind(this.db));
            const rows = await dbAll(`SELECT session_id, host_user_id, status, created_at, last_activity, config
         FROM swarm_sessions
     ...
     */
    async removeSwarmSession(sessionId) {
        if (!this.isInitialized || !this.db) {
            return;
        }
        try {
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            await dbRun(`DELETE FROM swarm_sessions WHERE session_id = ?`, sessionId);
     ...
     */
    async storeSwarmSessionParticipant(participant) {
        if (!this.isInitialized || !this.db) {
            return;
        }
        try {
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            await dbRun(`INSERT OR REPLACE INTO swarm_session_participants
         (session_id, user_id, agent_id, display_name, joined_at, is_host, capabilities, connection_state)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, participant.sessionId, participant.userId, participant.agentId, participant.displayName, participant.joinedAt.toISOString(), participant.isHost, participant.capabilities, participant.connectionState);
     ...
     */
    async getSwarmSessionParticipants(sessionId) {
        if (!this.isInitialized || !this.db) {
            return [];
        }
        try {
            const dbAll = (0, util_1.promisify)(this.db.all.bind(this.db));
            const rows = await dbAll(`SELECT session_id, user_id, agent_id, display_name, joined_at, is_host, capabilities, connection_state
     ...
     */
    async removeSwarmSessionParticipant(sessionId, userId) {
        if (!this.isInitialized || !this.db) {
            return;
        }
        try {
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            await dbRun(`DELETE FROM swarm_session_participants WHERE session_id = ? AND user_id = ?`, sessionId, userId);
     ...
     */
    async storeSwarmSessionTask(task) {
        if (!this.isInitialized || !this.db) {
            return;
        }
        try {
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            await dbRun(`INSERT OR REPLACE INTO swarm_session_tasks (session_id, task_id, task_data)
         VALUES (?, ?, ?)`, task.sessionId, task.taskId, task.taskData);
     ...
     */
    async getSwarmSessionTasks(sessionId) {
        if (!this.isInitialized || !this.db) {
            return [];
        }
        try {
            const dbAll = (0, util_1.promisify)(this.db.all.bind(this.db));
            const rows = await dbAll(`SELECT session_id, task_id, task_data
     ...
     */
    async removeSwarmSessionTask(sessionId, taskId) {
        if (!this.isInitialized || !this.db) {
            return;
        }
        try {
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            await dbRun(`DELETE FROM swarm_session_tasks WHERE session_id = ? AND task_id = ?`, sessionId, taskId);
     ...
     */
    async storeSwarmSessionVeto(veto) {
        if (!this.isInitialized || !this.db) {
            return;
        }
        try {
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            await dbRun(`INSERT OR REPLACE INTO swarm_session_vetos (session_id, task_id, voting_session_id)
         VALUES (?, ?, ?)`, veto.sessionId, veto.taskId, veto.votingSessionId);
     ...
     */
    async getSwarmSessionVetos(sessionId) {
        if (!this.isInitialized || !this.db) {
            return [];
        }
        try {
            const dbAll = (0, util_1.promisify)(this.db.all.bind(this.db));
            const rows = await dbAll(`SELECT session_id, task_id, voting_session_id
     ...
     */
    async removeSwarmSessionVeto(sessionId, taskId) {
        if (!this.isInitialized || !this.db) {
            return;
        }
        try {
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            await dbRun(`DELETE FROM swarm_session_vetos WHERE session_id = ? AND task_id = ?`, sessionId, taskId);
     ...
     */
    async storeSwarmSessionA2AHandshake(handshake) {
        if (!this.isInitialized || !this.db) {
            return;
        }
        try {
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            await dbRun(`INSERT OR REPLACE INTO swarm_session_a2a_handshakes (session_id, agent_pair, handshake_id)
         VALUES (?, ?, ?)`, handshake.sessionId, handshake.agentPair, handshake.handshakeId);
     ...
     */
    async getSwarmSessionA2AHandshakes(sessionId) {
        if (!this.isInitialized || !this.db) {
            return [];
        }
        try {
            const dbAll = (0, util_1.promisify)(this.db.all.bind(this.db));
            const rows = await dbAll(`SELECT session_id, agent_pair, handshake_id
     ...
     */
    async removeSwarmSessionA2AHandshake(sessionId, agentPair) {
        if (!this.isInitialized || !this.db) {
            return;
        }
        try {
            const dbRun = (0, util_1.promisify)(this.db.run.bind(this.db));
            await dbRun(`DELETE FROM swarm_session_a2a_handshakes WHERE session_id = ? AND agent_pair = ?`, sessionId, agentPair);
...
exports.AutoGenMemoriSQLite = AutoGenMemoriSQLite;
// Default export for convenience
exports.autoGenMemoriSQLite = new AutoGenMemoriSQLite();
//# sourceMappingURL=memori-sqlite.js.map