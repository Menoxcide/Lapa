...
exports.measureMemoriRecall = measureMemoriRecall;
exports.measureEpisodicRecall = measureEpisodicRecall;
exports.measureChromaRecall = measureChromaRecall;
exports.measureCombinedRecall = measureCombinedRecall;
exports.validateRecallTarget = validateRecallTarget;
exports.generateRecallReport = generateRecallReport;
const memori_engine_ts_1 = require("./memori-engine.ts");
const episodic_ts_1 = require("./episodic.ts");
const chroma_refine_ts_1 = require("../rag/chroma-refine.ts");
/**
 * Measures recall for Memori Engine entity retrieval
 */
async function measureMemoriRecall(testEntities, limit = 20) {
    const startTime = Date.now();
    // Store all test entities
    const storedEntityIds = [];
    for (const testEntity of testEntities) {
        const entities = await memori_engine_ts_1.memoriEngine.extractAndStoreEntities(testEntity.taskId, testEntity.content);
    ...
    for (const testEntity of testEntities) {
        const entities = await memori_engine_ts_1.memoriEngine.getContextEntities(testEntity.taskId, limit);
 ...
 */
async function measureEpisodicRecall(testEpisodes, limit = 50) {
    const startTime = Date.now();
    // Store all test episodes
    const storedEpisodeIds = [];
    for (const testEpisode of testEpisodes) {
        const episode = await episodic_ts_1.episodicMemoryStore.storeEpisode({
    ...
    for (const agentId of agentIds) {
        const episodes = await episodic_ts_1.episodicMemoryStore.getEpisodesByAgent(agentId, limit);
 ...
 */
async function measureChromaRecall(testDocuments, similarityThreshold = 0.7) {
    const startTime = Date.now();
    ...
    for (const doc of testDocuments) {
        const results = await chroma_refine_ts_1.chromaRefine.searchSimilar(doc.query, {
            limit: 10,
 ...
 */
async function measureCombinedRecall(testData) {
    // Measure each system
    ...
    const weightedRecall = totalItems > 0
        ...
        : 0;
    const targetMet = weightedRecall >= 0.995; // 99.5% target
    return {
 ...
 */
function validateRecallTarget(metrics) {
 ...
 */
function generateRecallReport(result) {
...
//# sourceMappingURL=recall-metrics.js.map