...
exports.FlowGuardsConfigSchema = exports.FlowGuardSchema = void 0;
exports.validateFlowGuard = validateFlowGuard;
exports.validateFlowGuardsConfig = validateFlowGuardsConfig;
exports.validateGuardContext = validateGuardContext;
const zod_1 = require("zod");
const flow_guards_ts_1 = require("../orchestrator/flow-guards.ts");
Object.defineProperty(exports, "FlowGuardSchema", { enumerable: true, get: function () { return flow_guards_ts_1.FlowGuardSchema; } });
Object.defineProperty(exports, "FlowGuardsConfigSchema", { enumerable: true, get: function () { return flow_guards_ts_1.FlowGuardsConfigSchema; } });
/**
 * Validates a single flow guard configuration
 * @param guard The flow guard to validate
 * @returns Validation result with errors and warnings
 */
function validateFlowGuard(guard) {
    const result = {
        isValid: true,
        errors: [],
        warnings: []
    ...
    try {
        ...
        const parsed = flow_guards_ts_1.FlowGuardSchema.safeParse(guard);
        if (!parsed.success) {
 ...
 * @returns Validation result with errors and warnings
 */
function validateFlowGuardsConfig(config) {
    const result = {
        isValid: true,
        errors: [],
        warnings: []
    ...
    try {
        // Validate against schema
        const parsed = flow_guards_ts_1.FlowGuardsConfigSchema.safeParse(config);
        if (!parsed.success) {
            result.isValid = false;
            result.errors = parsed.error.errors.map(err => ({
                path: err.path.join('.'),
 ...
 */
function validateStringCondition(condition) {
    const errors = [];
 ...
 * @returns Array of error messages
 */
function validateStringAction(action) {
    const errors = [];
 ...
 */
function validateGuardContext(context) {
    const result = {
        isValid: true,
        errors: [],
        warnings: []
...
//# sourceMappingURL=flow-guard-validation.js.map