...
exports.hybridHandoffSystem = exports.HybridHandoffSystem = exports.HANDOFF_CONFIG_PRESETS = exports.HandoffConfigValidationError = void 0;
...
const a2a_mediator_ts_1 = require("./a2a-mediator.ts");
 ...
 */
class HybridHandoffSystem {
    langGraphOrchestrator;
    contextHandoffManager;
    openAIAgents = new Map();
    config;
    thresholdManager;
    hooks = {};
    retryConfig;
                    ...
                    if (evaluation && evaluation.shouldHandoff && evaluation.targetAgentId) {
                        ...
                        if (shouldHandoff) {
                            try {
                                ...
                                const handoffResult = await this.initiateHandoff('current-agent', // source agent ID (placeholder)
                                evaluation.targetAgentId, context.task?.id || `task-${Date.now()}`, context);
            ...
            if (evaluatorAgent) {
                ...
                for (let attempt = 0; attempt < 2; attempt++) {
                    try {
                        ...
                        const evaluationResult = await (0, agents_1.run)(evaluatorAgent, evaluationPrompt);
     ...
     */
    async handoffToOpenAIAgent(targetAgent, taskId, context, targetAgentId, sourceAgentId) {
        ...
        try {
            ...
            const result = await this.runWithRetry(() => {
                return (0, agents_1.run)(targetAgent, `Process this task: ${handoffTask.input}`);
            }, `Handoff to OpenAI agent ${targetAgentId}`, sourceAgentId, targetAgentId, taskId);
     ...
     */
    async runWithRetry(operation, operationName, sourceAgentId, targetAgentId, taskId) {
        let lastError;
     ...
     */
    extractCapabilitiesFromTask(task) {
        const capabilities = [];
...
exports.HybridHandoffSystem = HybridHandoffSystem;
// Export singleton instance
exports.hybridHandoffSystem = new HybridHandoffSystem();
