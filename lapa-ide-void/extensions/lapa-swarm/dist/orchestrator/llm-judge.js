...
const judgmentRequestSchema = zod_1.z.object({
    type: zod_1.z.enum(['code-quality', 'hallucination', 'soc-violation', 'test-validity', 'other']),
...
class LLMJudge {
     ...
     */
    async getLLMJudgment(request) {
        try {
            const prompt = this.buildJudgmentPrompt(request);
            // Use Ollama for local LLM judgment via the inference module
            const reasoning = await (0, ollama_local_ts_1.sendOllamaChatRequest)(this.config.model, [{ role: 'user', content: prompt }], {
     ...
     */
    generateSuggestion(rule, result) {
