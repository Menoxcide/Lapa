...
exports.DEFAULT_MODE_BEHAVIORS = exports.ModeAwareAgentDecorator = void 0;
exports.createModeAwareAgent = createModeAwareAgent;
exports.initializeModeBehaviors = initializeModeBehaviors;
const modes_ts_1 = require("../modes/modes.ts");
const persona_manager_ts_1 = require("./persona.manager.ts");
/**
 * Mode-aware agent decorator
 */
class ModeAwareAgentDecorator {
    agent;
    modeBehaviors = new Map();
    modePersonas = new Map();
    currentModeBehavior = null;
    currentModePersona = null;
    constructor(agent) {
        this.agent = agent;
     ...
     */
    async adaptToCurrentMode() {
        const currentMode = modes_ts_1.rooModeController.getCurrentMode();
        ...
        if (personaId) {
            ...
            const persona = persona_manager_ts_1.personaManager.getPersona(personaId);
            if (persona) {
     ...
     */
    async executeTaskWithModeAdaptation(task) {
        ...
        if (this.currentModePersona) {
            const persona = persona_manager_ts_1.personaManager.getPersona(this.currentModePersona);
            if (persona) {
                const adjustedDescription = persona_manager_ts_1.personaManager.applyPersonaToContent(this.currentModePersona, task.description);
                processedTask = {
                    ...task,
        ...
        return {
            success: true,
            result: `Task executed by ${this.name} in ${modes_ts_1.rooModeController.getCurrentMode()} mode`,
            taskId: task.id
        };
    }
    // Add missing method to resolve test failures
    getCurrentMode() {
        return modes_ts_1.rooModeController.getCurrentMode();
    }
}
exports.ModeAwareAgentDecorator = ModeAwareAgentDecorator;
/**
 ...
 */
function initializeModeBehaviors(agent, agentType) {
    console.log(`Initializing mode behaviors for agent ${agent.name} (${agentType})`);
    // Set up mode behaviors for each mode
    for (const [mode, behaviors] of Object.entries(exports.DEFAULT_MODE_BEHAVIORS)) {
        const modeBehavior = behaviors[agentType];
        if (modeBehavior) {
            agent.setModeBehavior(mode, modeBehavior);
        }
    }
    ...
    for (const [mode, personas] of Object.entries(personaMappings)) {
        const personaId = personas[agentType];
        if (personaId) {
            agent.setModePersona(mode, personaId);
        }
    }
    console.log(`Mode behaviors initialized for agent ${agent.name}`);
}
//# sourceMappingURL=agent-mode-extension.js.map