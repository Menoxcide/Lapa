...
class Tester {
     ...
     */
    async generateTest(code, requirements) {
        try {
...
Please provide a complete test case that follows best practices and covers edge cases.`;
     ...
     * @returns Test result
     */
    async executeTest(testCode, implementationCode) {
     ...
     */
    async makeTestPass(failingTest, currentImplementation) {
        try {
...
Please provide a fixed implementation that makes the test pass while maintaining code quality and following best practices.`;
     ...
     */
    async suggestRefactorings(implementation, testResults) {
        try {
...
Format each suggestion with a description, priority (low/medium/high), and implementation details.`;
     ...
     */
    async runTDDCycle(requirements, currentImplementation) {
        const cycleResult = {
            cycle: Date.now(),
            redPhase: {
                testGenerated: ''
        ...
        try {
            ...
            cycleResult.greenPhase.implementation = await this.makeTestPass(cycleResult.redPhase.testGenerated, currentImplementation || '');
            // Execute the test again (should pass now)
            cycleResult.greenPhase.testResult = await this.executeTest(cycleResult.redPhase.testGenerated, cycleResult.greenPhase.implementation);
            // Refactor Phase: Suggest improvements
            console.log('REFACTOR PHASE: Suggesting improvements...');
            const refactorSuggestions = await this.suggestRefactorings(cycleResult.greenPhase.implementation, [cycleResult.greenPhase.testResult]);
