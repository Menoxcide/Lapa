...
const WelcomeTour = ({ onComplete, onSkip }) => {
        ...
        },
        {
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
                ...
                ].map(role => (<button key={role.id} onClick={() => setSelectedRole(role.id)} className={`p-4 border-2 rounded-lg transition-all ${selectedRole === role.id
        ...
        },
        {
          ...
          <div className="space-y-3">
            {[
                ...
                ].map(({ mode, desc }) => (<div key={mode} className="flex items-center p-3 bg-gray-50 rounded">
    ...
    return (<div className="welcome-tour fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-2xl font-bold">Welcome to LAPA</h2>
          {onSkip && (<button onClick={onSkip} className="text-gray-500 hover:text-gray-700 text-sm">
              Skip Tour
            </button>)}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="mb-4">
            ...
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div className="bg-blue-500 h-2 rounded-full transition-all" style={{ width: `${((currentStep + 1) / steps.length) * 100}%` }}/>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between p-6 border-t bg-gray-50">
          <button onClick={handlePrevious} disabled={currentStep === 0} className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed">
            Previous
          </button>
          <div className="flex gap-2">
            {steps.map((_, idx) => (<div key={idx} className={`w-2 h-2 rounded-full ${idx === currentStep ? 'bg-blue-500' : 'bg-gray-300'}`}/>))}
          </div>
