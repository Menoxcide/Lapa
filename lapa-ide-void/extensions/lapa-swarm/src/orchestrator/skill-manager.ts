...
export class SkillManager {
  ...
  private async discoverSkills(): Promise<void> {
    try {
      ...
      for (const entry of entries) {
        if (entry.isFile()) {
          if (entry.name.endsWith('.skill.ts')) {
            await this.loadSkillMetadata(join(skillsDir, entry.name));
          } else if (entry.name === 'skill.yaml' || entry.name === 'skill.yml') {
            // Load skill from YAML metadata file
            await this.loadSkillFromYaml(join(skillsDir, entry.name));
          }
      ...
      for (const entry of entries) {
        if (entry.isFile()) {
          if (entry.name.endsWith('.skill.ts')) {
            ...
            // Load skill from YAML metadata file
            await this.loadSkillFromYaml(join(dirPath, entry.name));
          } else if (entry.name === 'index.ts') {
            // Check if there's a skill.yaml in the same directory
            const yamlPath = join(dirPath, 'skill.yaml');
            try {
              await stat(yamlPath);
              // YAML exists, will be loaded by directory traversal
            } catch {
              // No YAML, try loading from index.ts directly
              await this.loadSkillMetadata(join(dirPath, entry.name));
            }
          }
        } else if (entry.isDirectory()) {
  ...
  /**
   * Loads skill from a YAML metadata file
   */
  private async loadSkillFromYaml(yamlPath: string): Promise<void> {
    try {
      const yamlContent = await readFile(yamlPath, 'utf-8');
      const metadata = parseYaml(yamlContent);
      
      // Find the corresponding implementation file
      const dirPath = yamlPath.includes('/') 
        ? yamlPath.substring(0, yamlPath.lastIndexOf('/'))
        : yamlPath.substring(0, yamlPath.lastIndexOf('\\'));
      const indexPath = join(dirPath, 'index.ts');
      
      let implementationPath = indexPath;
      try {
        await stat(indexPath);
      } catch {
        // Try alternative locations
        const skillName = basename(dirPath);
        const altPath = join(dirPath, `${skillName}.skill.ts`);
        try {
          await stat(altPath);
          implementationPath = altPath;
        } catch {
          console.warn(`[SkillManager] No implementation file found for skill ${metadata.id}`);
          implementationPath = indexPath; // Use default, will fail gracefully
        }
      }
      
      // Validate and extend metadata
      const extendedMetadata = {
        ...metadata,
        filePath: implementationPath,
        lastModified: (await stat(yamlPath)).mtime
      };
      
      const validated = skillMetadataSchema.parse(metadata);
      
      // Add the extended properties after validation
      (validated as any).filePath = implementationPath;
      (validated as any).lastModified = extendedMetadata.lastModified;
      
      this.skills.set(validated.id, validated as SkillMetadata);
      console.log(`[SkillManager] Loaded skill from YAML: ${validated.name} (${validated.id})`);
    } catch (error) {
      console.error(`[SkillManager] Failed to load skill from YAML ${yamlPath}:`, error);
    }
  }
