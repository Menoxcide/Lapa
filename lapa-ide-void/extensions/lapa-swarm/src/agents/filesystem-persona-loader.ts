/**
 * Filesystem Persona Loader for LAPA-VOID IDE Extension
 * 
 * Bridges NEURAFORGE filesystem persona system (docs/personas/*.md) with
 * runtime PersonaManager in the IDE extension. Enables loading personas from 
 * markdown files into the runtime agent system.
 * 
 * Adapted for VS Code extension context with proper path resolution.
 */
...
/**
 * Filesystem Persona Loader for IDE Extension
 * 
...
export class FilesystemPersonaLoader {
  ...
  private loadedPersonas: Map<string, ParsedPersonaFile> = new Map();
  private personaFilePaths: Map<string, string> = new Map();
  private workspaceRoot: string | null = null;

  constructor(config?: Partial<FilesystemPersonaConfig>) {
    // Determine workspace root from VS Code context
    const workspaceFolders = vscode.workspace.workspaceFolders;
    const defaultPersonasDir = workspaceFolders && workspaceFolders.length > 0
      ? join(workspaceFolders[0].uri.fsPath, 'docs', 'personas')
      : join(process.cwd(), 'docs', 'personas');

    this.config = {
      personasDirectory: defaultPersonasDir,
      enableAutoLoad: true,
  ...
  async loadAllPersonas(): Promise<number> {
    try {
      ...
      if (this.workspaceRoot) {
        const altPath = join(this.workspaceRoot, '..', 'docs', 'personas');
        try {
          const files = await readdir(altPath);
          const personaFiles = files.filter(
            file => file.endsWith('_PERSONA.md') || file.endsWith('_PERSONA.MD')
          );
          console.log(`Found ${personaFiles.length} personas in alternative path: ${altPath}`);
        } catch (altError) {
          console.warn('Alternative persona path also failed:', altError);
        }
      }
      return 0;
    }
  ...
  private async loadPersonaIntoRuntime(persona: Persona, filePath: string): Promise<void> {
    try {
      ...
      const existing = personaManager.getPersona(persona.id);
  ...
  /**
   * Get all loaded persona details
   */
  getAllPersonas(): ParsedPersonaFile[] {
    return Array.from(this.loadedPersonas.values());
  }
}

// Export singleton instance (will be initialized in extension activation)
let personaLoaderInstance: FilesystemPersonaLoader | null = null;

export function getPersonaLoader(): FilesystemPersonaLoader {
  if (!personaLoaderInstance) {
    personaLoaderInstance = new FilesystemPersonaLoader();
  }
