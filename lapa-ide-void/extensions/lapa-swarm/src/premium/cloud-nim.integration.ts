 ...
 * 
 * REQUIRES PREMIUM LICENSE: Cloud inference is a premium feature.
 */

// Import necessary modules
import { promisify } from 'util';
import { exec as execCallback } from 'child_process';
import { featureGate, PremiumFeature } from './feature-gate.ts';
...
export class CloudNIMIntegration {
    ...
    async sendInferenceRequest(
        ...
        // Check if cloud inference is enabled (premium feature)
        if (!featureGate.isPremiumFeatureEnabled(PremiumFeature.CLOUD_INFERENCE) &&
            !featureGate.isPremiumFeatureEnabled(PremiumFeature.CLOUD_NIM)) {
            throw new Error(
                'Cloud inference is a premium feature. Upgrade to Pro to enable cloud NIM integration. ' +
                'Free tier supports local inference only (Ollama/NIM local).'
            );
        }
