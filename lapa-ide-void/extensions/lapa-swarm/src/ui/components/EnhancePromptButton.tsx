/**
 * Enhance Prompt Button Component (Extension Version)
 * 
...
export const EnhancePromptButton: React.FC<EnhancePromptButtonProps> = ({
    ...
    try {
      ...
      if (vscode && vscode.postMessage) {
        ...
        const response = await new Promise<any>((resolve, reject) => {
          const messageId = `enhance-prompt-${Date.now()}`;
        ...
        if (response && response.refinedPrompt) {
          onEnhanced?.(response.refinedPrompt);
        } else if (response && response.clarificationQuestions) {
          const questions = response.clarificationQuestions.join('\n');
        ...
        // Fallback: try to import and use directly (if in same context)
        try {
          const { promptEngineer } = await import('../../orchestrator/prompt-engineer');
          ...
          console.error('[EnhancePromptButton] Direct import failed:', err);
          setError('Enhance prompt service not available');
        }
      }
    } catch (err) {
  ...
  return (
    <div className="relative inline-flex items-center">
      <button
        ...
        className={`
          ...
          ${disabled || !currentPrompt.trim() || isEnhancing
            ...
            : 'bg-blue-500 hover:bg-blue-600 text-white cursor-pointer hover:shadow-md dark:bg-blue-600 dark:hover:bg-blue-700'
          }
          ${className}
        `}
      ...
      {error && (
        <div className="absolute top-full left-0 mt-1 px-2 py-1 bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-200 text-xs rounded shadow-lg z-50 whitespace-nowrap">
          {error}
        </div>
      )}
    </div>
