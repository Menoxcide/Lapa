...
import { routeToMultilingualModel } from './multilingual-router.ts';
...
export type InferenceBackend = 'ollama' | 'nim' | 'auto' | 'aya' | 'command-r';
...
export interface InferenceManagerConfig {
  ...
  enableMultilingualRouting?: boolean;
}
...
export class InferenceManager {
  ...
  private commandRModel?: CommandRModel;
  ...
  async initialize(): Promise<void> {
    try {
      ...
      // Initialize multilingual models if API keys provided
      if (this.config.ayaApiKey) {
        this.ayaModel = new AyaModel({
          apiKey: this.config.ayaApiKey
        });
        await this.ayaModel.initialize();
      }

      if (this.config.commandRApiKey) {
        this.commandRModel = new CommandRModel({
          apiKey: this.config.commandRApiKey
        });
        await this.commandRModel.initialize();
      }
  ...
  private async ensureBackendAvailable(backend: InferenceBackend): Promise<void> {
    // Check if backend is Aya or Command-R (API-based, no local setup needed)
    if (backend === 'aya' || backend === 'command-r') {
      return; // API models are always "available" if API key is set
    }
    if (backend === 'auto') {
  ...
  async infer(request: InferenceRequest): Promise<InferenceResponse> {
    ...
    try {
      ...
      let actualBackend: InferenceBackend = backend;
      
      // Use multilingual routing if enabled
      if (this.config.enableMultilingualRouting && backend === 'auto') {
        const routing = await routeToMultilingualModel(
          {
            description: request.prompt,
            context: request.context,
            filePaths: request.parameters?.filePaths as string[] || []
          },
          {
            ayaApiKey: this.config.ayaApiKey,
            commandRApiKey: this.config.commandRApiKey,
            preferLocal: true
          }
        );
        actualBackend = routing.model;
      }

      if (actualBackend === 'aya' && this.ayaModel) {
        const ayaResponse = await this.ayaModel.generate({
          prompt: request.prompt,
          maxTokens: request.parameters?.maxTokens || 1024,
          temperature: request.parameters?.temperature || 0.7
        });
        response = ayaResponse.text;
        tokensUsed = ayaResponse.tokensUsed;
      } else if (actualBackend === 'command-r' && this.commandRModel) {
        const commandRResponse = await this.commandRModel.chat({
          messages: request.messages || [{ role: 'user', content: request.prompt }],
          context: request.context,
          maxTokens: request.parameters?.maxTokens || 4096,
          temperature: request.parameters?.temperature || 0.7
        });
        response = commandRResponse.text;
        tokensUsed = commandRResponse.tokensUsed;
      } else if (actualBackend === 'nim' || (actualBackend === 'auto' && this.currentBackend === 'nim')) {
