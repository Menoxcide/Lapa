...
interface IntegrationTestContext {
  visionVoiceController: VisionVoiceController;
  visionAgentTool: VisionAgentTool;
  voiceAgentTool: VoiceAgentTool;
  agentWrapper: HelixTeamAgentWrapper;
...
describe('Multimodal Agent Integration', () => {
  ...
  let testRunner: IntegrationTestSuiteRunner;
  ...
  beforeEach(() => {
    ...
    const visionVoiceController = new VisionVoiceController(config);
    const visionAgentTool = new VisionAgentTool(config);
    const voiceAgentTool = new VoiceAgentTool();
    ...
    const agentWrapper = new HelixTeamAgentWrapper(
      agentConfig.id,
      agentConfig.type,
      agentConfig.name,
      agentConfig.capabilities,
      agentConfig.workload,
      agentConfig.capacity,
      agentToolRegistry
    );
  ...
  describe('Agent Tool Execution Integration', () => {
    it('should execute vision agent tool through agent wrapper', async () => {
      const result = await testRunner.runTest('execute_vision_tool_via_wrapper', async () => {
        ...
        const executionResult: AgentToolExecutionResult = await testContext.agentWrapper.executeTool('vision-agent', context);
    ...
    it('should execute voice agent tool through agent wrapper', async () => {
      const result = await testRunner.runTest('execute_voice_tool_via_wrapper', async () => {
        ...
        const executionResult: AgentToolExecutionResult = await testContext.agentWrapper.executeTool('voice-agent', context);
    ...
    it('should handle invalid tool execution gracefully', async () => {
      const result = await testRunner.runTest('handle_invalid_tool_execution', async () => {
        ...
        const executionResult: AgentToolExecutionResult = await testContext.agentWrapper.executeTool('non-existent-tool', context);
  ...
  describe('Event Bus Integration', () => {
    ...
    it('should handle event bus errors gracefully', async () => {
      const result = await testRunner.runTest('handle_event_bus_errors', async () => {
        ...
        const mockPublish = vi.spyOn(eventBus, 'publish').mockImplementationOnce(() => {
    ...
    it('should handle concurrent tool executions', async () => {
      const result = await testRunner.runTest('handle_concurrent_executions', async () => {
        ...
        const results = await Promise.all(
          contexts.map(context => testContext.agentWrapper.executeTool(context.toolName, context))
        );
  ...
  describe('Error Handling and Recovery', () => {
    it('should recover from tool execution errors', async () => {
      const result = await testRunner.runTest('recover_from_tool_errors', async () => {
        ...
        const validContext: AgentToolExecutionContext = {
          toolName: 'vision-agent',
          parameters: {
            action: 'processImage',
            imageData: 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg=='
        ...
        const validResult: AgentToolExecutionResult = await testContext.agentWrapper.executeTool('vision-agent', validContext);
