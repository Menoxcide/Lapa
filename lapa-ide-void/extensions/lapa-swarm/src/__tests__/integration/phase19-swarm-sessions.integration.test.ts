...
describe('Phase 19 Swarm Sessions Integration', () => {
  ...
  describe('WebRTC Session Management', () => {
    it('should create and manage swarm sessions with WebRTC', async () => {
      ...
      // Create session
      const sessionId = await createSwarmSession(config, ADMIN_USER);
      ...
      expect(session?.hostUserId).toBe(ADMIN_USER);
      expect(session?.participants.size).toBe(1);

      // Join with another participant
      const joined = await joinSwarmSession(sessionId, REGULAR_USER, 'Regular User');
      expect(joined).toBe(true);

      // Verify participant was added
      const updatedSession = swarmSessionManager.getSession(sessionId);
      expect(updatedSession?.participants.size).toBe(2);
      expect(updatedSession?.participants.has(REGULAR_USER)).toBe(true);

      // Leave session
      await swarmSessionManager.leaveSession(sessionId, REGULAR_USER);

      // Verify participant was removed
      const finalSession = swarmSessionManager.getSession(sessionId);
      expect(finalSession?.participants.size).toBe(1);
      expect(finalSession?.participants.has(REGULAR_USER)).toBe(false);

      // Close session
      await swarmSessionManager.closeSession(sessionId);
      const closedSession = swarmSessionManager.getSession(sessionId);
      expect(closedSession).toBeUndefined();
    });

    it('should handle WebRTC connection establishment between participants', async () => {
      const config: SessionConfig = {
      ...
      expect(iceCandidates.length).toBeGreaterThanOrEqual(0);
    ...
    it('should handle session errors gracefully', async () => {
      ...
      await expect(createSwarmSession(config, UNAUTHORIZED_USER))
        .rejects
        .toThrow(/permission/);

      // Try to join non-existent session
      await expect(joinSwarmSession('non-existent-session', REGULAR_USER, 'Regular User'))
        .rejects
        .toThrow(/not found/);
      ...
      await expect(joinSwarmSession(sessionId, REGULAR_USER, 'Regular User'))
        .rejects
        .toThrow(/full/);
    });
  });

  describe('Signaling Server Functionality with NAT Traversal', () => {
    it('should handle participant joining and session creation through signaling server', async () => {
      // Verify signaling server is running
      expect(signalingServer).toBeDefined();
      expect(signalingServer.getSessionCount()).toBe(0);
      ...
      expect(signalingServer.getParticipantCount()).toBe(0);
    });
  ...
  describe('RBAC Security Integration', () => {
    it('should enforce RBAC permissions for session operations', async () => {
      ...
      await expect(createSwarmSession({
      ...
      await expect(joinSwarmSession(sessionId, UNAUTHORIZED_USER, 'Unauthorized User'))
        .rejects
        .toThrow(/permission/);
    });

    it('should enforce RBAC permissions for veto operations', async () => {
      const sessionId = await createSwarmSession({
      ...
      await joinSwarmSession(sessionId, REGULAR_USER, 'Regular User');
      ...
      const adminVetoResponse = await swarmSessionManager.requestVeto(
        sessionId, 'task-1', ADMIN_USER, 'Admin veto reason'
      );
      expect(adminVetoResponse).toBeDefined();

      // Regular user should be able to request veto
      const regularVetoResponse = await swarmSessionManager.requestVeto(
        sessionId, 'task-1', REGULAR_USER, 'Regular user veto reason'
      );
      expect(regularVetoResponse).toBeDefined();

      // Unauthorized user should not be able to request veto
      const unauthorizedVetoResponse = await swarmSessionManager.requestVeto(
        sessionId, 'task-1', UNAUTHORIZED_USER, 'Unauthorized veto reason'
      );
      expect(unauthorizedVetoResponse.accepted).toBe(false);
      expect(unauthorizedVetoResponse.reason).toContain('permission');
    });

    it('should enforce RBAC permissions for A2A operations', async () => {
      const sessionId = await createSwarmSession({
      ...
      if (session) {
        ...
        if (adminParticipant && regularParticipant) {
          ...
          const handshakeResponse = await swarmSessionManager.initiateA2AHandshake(
            sessionId, 'agent-admin', 'agent-regular', task
          );
          
          expect(handshakeResponse).toBeDefined();
  ...
  describe('Session Persistence and Recovery', () => {
    it('should persist session data to memori engine', async () => {
      const sessionId = await createSwarmSession({
      ...
      const sessionCreatedSubscription = eventBus.subscribe('swarm.session.created', () => {
        sessionCreatedEvent = true;
      });

      const participantJoinedSubscription = eventBus.subscribe('swarm.session.participant.joined', () => {
        participantJoinedEvent = true;
      });

      const taskCompletedSubscription = eventBus.subscribe('swarm.task.completed', () => {
        taskAddedEvent = true;
      });
      ...
      expect(sessionCreatedEvent).toBe(true);
      expect(participantJoinedEvent).toBe(true);

      // Cleanup subscriptions
      eventBus.unsubscribe(sessionCreatedSubscription);
      eventBus.unsubscribe(participantJoinedSubscription);
      eventBus.unsubscribe(taskCompletedSubscription);
    });

    it('should recover sessions from persistence', async () => {
  ...
  describe('Message Handler Functionality', () => {
    it('should handle task messages correctly', async () => {
      const sessionId = await createSwarmSession({
      ...
      // @ts-ignore - accessing private method for testing
      await swarmSessionManager.handleTaskMessage(session, completionMessage);

      // Verify task was removed
      expect(session.activeTasks.get('task-1')).toBeUndefined();
    });

    it('should handle veto messages correctly', async () => {
      const sessionId = await createSwarmSession({
      ...
      const vetoSubscription = eventBus.subscribe('swarm.task.vetoed', () => {
      ...
      eventBus.unsubscribe(vetoSubscription);

      // Verify function completed (with mock voting, we can't predict outcome)
      expect(true).toBe(true);
    });

    it('should handle A2A messages correctly', async () => {
      const sessionId = await createSwarmSession({
    ...
    it('should handle state synchronization messages correctly', async () => {
      const sessionId = await createSwarmSession({
    ...
    it('should handle handoff messages correctly', async () => {
      const sessionId = await createSwarmSession({
  ...
  describe('Cross-Environment Compatibility', () => {
    ...
    it('should handle browser-like WebSocket interactions', async () => {
