...
import { createModeAwareAgent, initializeModeBehaviors } from '../../agents/agent-mode-extension.ts';
import { moeRouter, Agent } from '../../agents/moe-router.ts';
import { eventBus } from '../../core/event-bus.ts';
import { vi } from 'vitest';

// Mock agents for testing
const mockCoderAgent: Agent = {
  id: 'mock-coder-1',
  type: 'coder',
...
describe('Roo Mode Controller Integration', () => {
  let controller: RooModeController;
  
  beforeAll(async () => {
    // Initialize the mode controller
    controller = rooModeController;
  ...
  describe('Mode Switching', () => {
    it('should successfully switch from ask mode to code mode', async () => {
      ...
      // Act
      const result: ModeTransitionResult = await controller.requestModeChange(request);
  ...
  describe('Agent Mode Adaptation', () => {
    it('should adapt agent behavior when mode changes to code', async () => {
      ...
      const modeAwareAgent = createModeAwareAgent(mockCoderAgent);
      initializeModeBehaviors(modeAwareAgent, 'coder');
    ...
    it('should adapt agent behavior when mode changes to architect', async () => {
      // Arrange
      // Create a mode-aware agent
      const modeAwareAgent = createModeAwareAgent(mockPlannerAgent);
      initializeModeBehaviors(modeAwareAgent, 'planner');
  ...
  describe('Event Integration', () => {
    it('should publish mode change events', async () => {
      ...
      eventBus.subscribe('mode.changed', eventHandler);
  ...
  describe('Mode Configuration', () => {
    ...
    it('should return undefined for non-existent mode configuration', () => {
      // Act
      const config = controller.getModeConfig('non-existent-mode' as RooMode);
