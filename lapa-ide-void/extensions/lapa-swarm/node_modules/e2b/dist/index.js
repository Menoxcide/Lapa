  ...
  for (const file of globFiles) {
    if (file.isDirectory()) {
      ...
      const dirFiles = await glob(
        import_node_path.default.join(import_node_path.default.relative(contextPath, file.fullpath()), "**/*"),
        {
          ignore: ignorePatterns,
          withFileTypes: true,
          cwd: contextPath
...
async function tarFileStream(fileName, fileContextPath, ignorePatterns, resolveSymlinks) {
  ...
  const filePaths = allFiles.map(
    (file) => import_node_path.default.relative(fileContextPath, file.fullpath())
  );
  return create(
    {
      gzip: true,
