...
import { LAPA_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, VoidStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './lapaSettingsTypes.js';
...
// type RealLAPASettings = Exclude<keyof LAPASettingsState, '_modelOptions'>
// type EventProp<T extends RealLAPASettings = RealLAPASettings> = T extends 'globalSettings' ? [T, keyof LAPASettingsState[T]] : T | 'all'


export interface ILAPASettingsService {
	readonly _serviceBrand: undefined;
	readonly state: LAPASettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: LAPASettingsState): Promise<void>;
...
const _stateWithMergedDefaultModels = (state: LAPASettingsState): LAPASettingsState => {
...
const _validatedModelState = (state: Omit<LAPASettingsState, '_modelOptions'>): LAPASettingsState => {
...
export const ILAPASettingsService = createDecorator<ILAPASettingsService>('LAPASettingsService');
class LAPASettingsService extends Disposable implements ILAPASettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: LAPASettingsState;
	...
	dangerousSetState = async (newState: LAPASettingsState) => {
	...
	private async _readState(): Promise<LAPASettingsState> {
		const encryptedState = this._storageService.get(LAPA_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(LAPA_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: LAPASettingsState = {
			...this.state,
			globalSettings: {
...
registerSingleton(ILAPASettingsService, LAPASettingsService, InstantiationType.Eager);
