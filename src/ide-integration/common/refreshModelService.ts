...
import { ILAPASettingsService } from './lapaSettingsService.js';
import { ILLMMessageService } from './sendLLMMessageService.js';
import { Emitter, Event } from '../../../../base/common/event.js';
import { Disposable, IDisposable } from '../../../../base/common/lifecycle.js';
import { RefreshableProviderName, refreshableProviderNames, SettingsOfProvider } from './lapaSettingsTypes.js';
...
export class RefreshModelService extends Disposable implements IRefreshModelService {
	...
	constructor(
		@ILAPASettingsService private readonly lapaSettingsService: ILAPASettingsService,
		...
		const initializeAutoPollingAndOnChange = () => {
			...
			if (!lapaSettingsService.state.globalSettings.autoRefreshModels) return

			for (const providerName of refreshableProviderNames) {

				// const { '_didFillInProviderSettings': enabled } = this.lapaSettingsService.state.settingsOfProvider[providerName]
				this.startRefreshingModels(providerName, autoOptions)

				// every time providerName.enabled changes, refresh models too, like a useEffect
				let relevantVals = () => refreshBasedOn[providerName].map(settingName => lapaSettingsService.state.settingsOfProvider[providerName][settingName])
				let prevVals = relevantVals() // each iteration of a for loop has its own context and vars, so this is ok
				disposables.add(
					lapaSettingsService.onDidChangeState(() => { // we might want to debounce this
		...
		lapaSettingsService.waitForInitState.then(() => {
			initializeAutoPollingAndOnChange()
			this._register(
				lapaSettingsService.onDidChangeState((type) => { if (typeof type === 'object' && type[1] === 'autoRefreshModels') initializeAutoPollingAndOnChange() })
			)
		})
	...
	startRefreshingModels: IRefreshModelService['startRefreshingModels'] = (providerName, options) => {
		...
		const autoPoll = () => {
			if (this.lapaSettingsService.state.globalSettings.autoRefreshModels) {
				// resume auto-polling
		...
		listFn({
			...
			onSuccess: ({ models }) => {
				...
				this.lapaSettingsService.setAutodetectedModels(
					providerName,
					models.map(model => {
				...
				if (options.enableProviderOnSuccess) this.lapaSettingsService.setSettingOfProvider(providerName, '_didFillInProviderSettings', true)
