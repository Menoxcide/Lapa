...
import { MCPUserStateOfName } from './lapaSettingsTypes.js';
...
class MCPService extends Disposable implements IMCPService {
	...
	private async _initialize() {
		try {
			await this.lapaSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.lapaSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.lapaSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.lapaSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		...
		await this.lapaSettingsService.setMCPServerState(serverName, { isOn });
