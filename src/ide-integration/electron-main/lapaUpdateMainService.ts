...
export class VoidMainUpdateService extends Disposable implements ILAPAUpdateService {
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<VoidCheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/Menoxcide/Lapa/releases/latest');
