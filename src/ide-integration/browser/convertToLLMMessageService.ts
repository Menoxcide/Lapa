...
import { ChatMode, FeatureName, ModelSelection, ProviderName } from '../common/lapaSettingsTypes.js';
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	constructor(
		...
		@ILAPAModelService private readonly lapaModelService: ILAPAModelService,
	...
	private _getVoidRulesFileContents(): string {
		try {
			...
			for (const folder of workspaceFolders) {
				...
				const { model } = this.lapaModelService.getModel(uri)
				if (!model) continue
	...
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.lapaSettingsService.state.globalSettings.aiInstructions;
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.lapaSettingsService.state
		...
		const modelSelectionOptions = this.lapaSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { disableSystemMessage } = this.lapaSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.lapaSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
