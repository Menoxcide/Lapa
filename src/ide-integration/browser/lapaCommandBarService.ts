...
import { mountLapaCommandBar } from './react/out/void-editor-widgets-tsx/index.js'
...
import { LAPA_ACCEPT_DIFF_ACTION_ID, LAPA_REJECT_DIFF_ACTION_ID, LAPA_GOTO_NEXT_DIFF_ACTION_ID, LAPA_GOTO_PREV_DIFF_ACTION_ID, LAPA_GOTO_NEXT_URI_ACTION_ID, LAPA_GOTO_PREV_URI_ACTION_ID, LAPA_ACCEPT_FILE_ACTION_ID, LAPA_REJECT_FILE_ACTION_ID, LAPA_ACCEPT_ALL_DIFFS_ACTION_ID, LAPA_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
import { ILAPAModelService } from '../common/lapaModelService.js';



export interface ILAPACommandBarService {
...
export const ILAPACommandBarService = createDecorator<ILAPACommandBarService>('LAPACommandBarService');
...
export class LAPACommandBarService extends Disposable implements ILAPACommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'lapa.LAPACommandBarService'
	...
	constructor(
		...
		@ILAPAModelService private readonly _lapaModelService: ILAPAModelService,
	) {
		super();
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._lapaModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(ILAPACommandBarService, LAPACommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type LapaCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountLapaCommandBar(root, accessor, { uri, editor } satisfies LapaCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies LapaCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const commandBarService = accessor.get(ILAPACommandBarService);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: LAPA_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: LAPA_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: LAPA_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: LAPA_GOTO_PREV_URI_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: LAPA_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: LAPA_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
