...
import { LAPA_VIEW_CONTAINER_ID, LAPA_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { LAPA_TOGGLE_SETTINGS_ACTION_ID } from './lapaSettingsPane.js';
...
const LAPA_OPEN_SIDEBAR_ACTION_ID = 'lapa.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: LAPA_OPEN_SIDEBAR_ACTION_ID, title: localize2('voidOpenSidebar', 'Void: Open Sidebar'), f1: true });
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const viewsService = accessor.get(IViewsService)
		const chatThreadsService = accessor.get(IChatThreadService)
		viewsService.openViewContainer(LAPA_VIEW_CONTAINER_ID)
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(LAPA_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(LAPA_OPEN_SIDEBAR_ACTION_ID)
		}
...
const LAPA_CMD_SHIFT_L_ACTION_ID = 'lapa.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: LAPA_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', LAPA_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		commandService.executeCommand(LAPA_CMD_SHIFT_L_ACTION_ID)
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(LAPA_TOGGLE_SETTINGS_ACTION_ID)
	}
})
