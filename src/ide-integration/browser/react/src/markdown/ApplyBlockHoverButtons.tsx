...
import { isFeatureNameDisabled } from '../../../../common/lapaSettingsTypes.js'
...
export const useApplyStreamState = ({ applyBoxId }: { applyBoxId: string }) => {
	const accessor = useAccessor()
	const lapaCommandBarService = accessor.get('ILAPACommandBarService')

	const getStreamState = useCallback(() => {
		const uri = getUriBeingApplied(applyBoxId)
		if (!uri) return 'idle-no-changes'
		return lapaCommandBarService.getStreamState(uri)
	}, [lapaCommandBarService, applyBoxId])
...
export const useEditToolStreamState = ({ applyBoxId, uri }: { applyBoxId: string, uri: URI }) => {
	const accessor = useAccessor()
	const lapaCommandBarService = accessor.get('ILAPACommandBarService')
	const [streamState, setStreamState] = useState(lapaCommandBarService.getStreamState(uri))
	// listen for stream updates on this box
	useCommandBarURIListener(useCallback((uri_) => {
		const shouldUpdate = uri.fsPath === uri_.fsPath
		if (shouldUpdate) { setStreamState(lapaCommandBarService.getStreamState(uri)) }
	}, [lapaCommandBarService, applyBoxId, uri]))
