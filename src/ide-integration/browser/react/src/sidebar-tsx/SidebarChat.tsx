...
import { LAPA_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { LAPA_OPEN_SETTINGS_ACTION_ID } from '../../../lapaSettingsPane.js';
import { ChatMode, displayInfoOfProviderName, FeatureName, isFeatureNameDisabled } from '../../../../../../../workbench/contrib/lapa/common/lapaSettingsTypes.js';
...
let SwarmControlToolbar: any = null;
...
try {
	...
	const swarmToolbar = require('../../../../../../extensions/lapa-swarm/src/ui/components/SwarmControlToolbar.js');
	SwarmControlToolbar = swarmToolbar.SwarmControlToolbar;
...
const ReasoningOptionSlider = ({ featureName }: { featureName: FeatureName }) => {
	...
	const lapaSettingsService = accessor.get('ILAPASettingsService')
	const lapaSettingsState = useSettingsState()

	const modelSelection = lapaSettingsState.modelSelectionOfFeature[featureName]
	const overridesOfModel = lapaSettingsState.overridesOfModel
	...
	const modelSelectionOptions = lapaSettingsState.optionsOfModelSelection[featureName][providerName]?.[modelName]
		...
		return <div className='flex items-center gap-x-2'>
			...
			<VoidSwitch
				...
				onChange={(newVal) => {
					...
					lapaSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff })
				}}
			/>
		</div>
	}
	...
	if (reasoningBudgetSlider?.type === 'budget_slider') { // if it's a slider
		...
		const value = isReasoningEnabled ? lapaSettingsState.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]?.reasoningBudget ?? defaultVal
			: valueIfOff
		...
		return <div className='flex items-center gap-x-2'>
			...
			<VoidSlider
				...
				onChange={(newVal) => {
					...
					lapaSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningBudget: newVal })
				}}
			/>
	...
	if (reasoningBudgetSlider?.type === 'effort_slider') {
		...
		const currentEffort = lapaSettingsState.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]?.reasoningEffort ?? defaultVal
		const valueIfOff = -1
		...
		return <div className='flex items-center gap-x-2'>
			...
			<VoidSlider
				...
				onChange={(newVal) => {
					const isOff = canTurnOffReasoning && newVal === valueIfOff
					lapaSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningEffort: values[newVal] ?? undefined })
				}}
			/>
...
const ChatModeDropdown = ({ className }: { className: string }) => {
	...
	const onChangeOption = useCallback((newVal: ChatMode) => {
		lapaSettingsService.setGlobalSetting('chatMode', newVal)
	}, [lapaSettingsService])
...
export const SelectedFiles = (
	...
	const accessor = useAccessor()
	const commandService = accessor.get('ICommandService')
	const modelReferenceService = accessor.get('ILAPAModelService')
...
export const SidebarChat = () => {
	...
	const keybindingString = accessor.get('IKeybindingService').lookupKeybinding(LAPA_CTRL_L_ACTION_ID)?.getLabel()
	...
	>
		...
		{latestError === undefined ? null :
			<div className='px-2 my-1'>
				...
				<WarningBox className='text-sm my-2 mx-4' onClick={() => { commandService.executeCommand(LAPA_OPEN_SETTINGS_ACTION_ID) }} text='Open settings' />
			</div>
		}
	...
	// LAPA Swarm Commands (Tier 1 - Critical)
	const handleStartSwarm = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.swarm.start')
		} catch (error) {
			console.error('[SidebarChat] Failed to start swarm:', error)
		}
	}, [commandService])
	...
	const handleStopSwarm = useCallback(async () => {
		try {
			...
			console.error('[SidebarChat] Failed to stop swarm:', error)
		}
	}, [commandService])
	...
	const handlePauseSwarm = useCallback(async () => {
		try {
			...
			console.error('[SidebarChat] Failed to pause swarm:', error)
		}
	}, [commandService])

	const handleResumeSwarm = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.swarm.resume')
		} catch (error) {
			console.error('[SidebarChat] Failed to resume swarm:', error)
		}
	}, [commandService])

	const handleSwarmStatus = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.swarm.status')
		} catch (error) {
			console.error('[SidebarChat] Failed to get swarm status:', error)
		}
	}, [commandService])

	const handleConfigureSwarm = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.swarm.configure')
		} catch (error) {
			console.error('[SidebarChat] Failed to configure swarm:', error)
		}
	}, [commandService])

	// LAPA Essential Commands (Tier 2)
	const handleEnhancePrompt = useCallback(async (prompt: string) => {
		try {
			const enhanced = await commandService.executeCommand('lapa.enhancePrompt', prompt) as string
			return enhanced
		} catch (error) {
			console.error('[SidebarChat] Failed to enhance prompt:', error)
			return prompt
		}
	}, [commandService])
	...
	const handleGenerateCommit = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.git.generateCommit')
		} catch (error) {
			console.error('[SidebarChat] Failed to generate commit:', error)
		}
	}, [commandService])

	const handleOpenSettings = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.settings.open')
		} catch (error) {
			console.error('[SidebarChat] Failed to open settings:', error)
		}
	}, [commandService])

	const handleOpenDashboard = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.dashboard.open')
		} catch (error) {
			console.error('[SidebarChat] Failed to open dashboard:', error)
		}
	}, [commandService])

	// LAPA Session Commands (Tier 3)
	const handleRestoreSession = useCallback(async (sessionId?: string) => {
		try {
			await commandService.executeCommand('lapa.swarm.restore', sessionId)
		} catch (error) {
			console.error('[SidebarChat] Failed to restore session:', error)
		}
	}, [commandService])

	const handleListSessions = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.swarm.listSessions')
		} catch (error) {
			console.error('[SidebarChat] Failed to list sessions:', error)
		}
	}, [commandService])

	// LAPA Persona Commands (Tier 3)
	const handleListPersonas = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.personas.list')
		} catch (error) {
			console.error('[SidebarChat] Failed to list personas:', error)
		}
	}, [commandService])

	const handleReloadPersonas = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.personas.reload')
		} catch (error) {
			console.error('[SidebarChat] Failed to reload personas:', error)
		}
	}, [commandService])

	// LAPA Workflow Commands (Tier 3)
	const handleGenerateWorkflow = useCallback(async (description?: string) => {
		try {
			await commandService.executeCommand('lapa.workflow.generate', description)
		} catch (error) {
			console.error('[SidebarChat] Failed to generate workflow:', error)
		}
	}, [commandService])

	// LAPA UI Navigation Commands (Tier 2-4)
	const handleOpenMarketplace = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.marketplace.open')
		} catch (error) {
			console.error('[SidebarChat] Failed to open marketplace:', error)
		}
	}, [commandService])

	const handleOpenROI = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.roi.open')
		} catch (error) {
			console.error('[SidebarChat] Failed to open ROI:', error)
		}
	}, [commandService])

	const handleOpenTaskHistory = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.taskHistory.open')
		} catch (error) {
			console.error('[SidebarChat] Failed to open task history:', error)
		}
	}, [commandService])

	// LAPA Command Palette Commands (Tier 4)
	const handleAISearch = useCallback(async (query?: string) => {
		try {
			await commandService.executeCommand('lapa.commandPalette.ai', query)
		} catch (error) {
			console.error('[SidebarChat] Failed to execute AI search:', error)
		}
	}, [commandService])

	// LAPA Premium Commands (Tier 5)
	const handleUpgrade = useCallback(async () => {
		try {
			await commandService.executeCommand('lapa.swarm.upgrade')
		} catch (error) {
			console.error('[SidebarChat] Failed to upgrade:', error)
		}
	}, [commandService])

	const handleActivateLicense = useCallback(async (licenseKey: string) => {
		try {
			await commandService.executeCommand('lapa.swarm.activateLicense', licenseKey)
		} catch (error) {
			console.error('[SidebarChat] Failed to activate license:', error)
		}
	}, [commandService])

	// Create toolbar component with swarm controls
	const toolbarContent = (
		<div className="flex flex-col gap-2">
			{SwarmControlToolbar && (
				<SwarmControlToolbar
					onStart={handleStartSwarm}
					onStop={handleStopSwarm}
					onPause={handlePauseSwarm}
					onResume={handleResumeSwarm}
					onStatus={handleSwarmStatus}
					onConfigure={handleConfigureSwarm}
					disabled={isDisabled}
					isRunning={false} // TODO: Get actual swarm state
					isPaused={false} // TODO: Get actual swarm state
					size="sm"
				/>
			)}
			{ChatInputToolbar && (
