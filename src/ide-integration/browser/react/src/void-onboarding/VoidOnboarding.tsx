...
import { displayInfoOfProviderName, ProviderName, providerNames, localProviderNames, featureNames, FeatureName, isFeatureNameDisabled } from '../../../../common/lapaSettingsTypes.js';
...
export const LapaOnboarding = () => {

	const lapaSettingsState = useSettingsState()
	const isOnboardingComplete = lapaSettingsState.globalSettings.isOnboardingComplete || OVERRIDE_VALUE
...
const LapaIcon = () => {
	...
	return <div ref={divRef} className='lapa-icon' />
}
...
const LapaOnboardingContent = () => {
	...
	const didFillInProviderSettings = selectedProviderName && lapaSettingsState.settingsOfProvider[selectedProviderName]._didFillInProviderSettings
	const isApiKeyLongEnoughIfApiKeyExists = selectedProviderName && lapaSettingsState.settingsOfProvider[selectedProviderName].apiKey ? lapaSettingsState.settingsOfProvider[selectedProviderName].apiKey.length > 15 : true
	const isAtLeastOneModel = selectedProviderName && lapaSettingsState.settingsOfProvider[selectedProviderName].models.length >= 1
	...
	const lastPagePrevAndNextButtons = <div className="max-w-[600px] w-full mx-auto flex flex-col items-end">
		<div className="flex items-center gap-2">
			...
			<PrimaryActionButton
				onClick={() => {
					lapaSettingsService.setGlobalSetting('isOnboardingComplete', true);
					voidMetricsService.capture('Completed Onboarding', { selectedProviderName, wantToUseOption })
				}}
				ringSize={lapaSettingsState.globalSettings.isOnboardingComplete ? 'screen' : undefined}
	...
	const detailedDescOfWantToUseOption: { [wantToUseOption in WantToUseOption]: string } = {
		...
		private: "Private-hosted so your data never leaves your computer or network. [Contact us](https://github.com/Menoxcide/Lapa/issues) for help setting up at your company.",
	...
	useEffect(() => {
		if (!lapaSettingsState.globalSettings.isOnboardingComplete) {
			setPageIndex(0)
		}
	}, [setPageIndex, lapaSettingsState.globalSettings.isOnboardingComplete])
