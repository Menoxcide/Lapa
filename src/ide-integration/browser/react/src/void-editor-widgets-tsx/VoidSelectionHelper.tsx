...
import '../styles.css'
import { LAPA_CTRL_K_ACTION_ID, LAPA_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { Circle, MoreVertical } from 'lucide-react';
import { useEffect, useState } from 'react';

import { LapaSelectionHelperProps } from '../../../../../../contrib/lapa/browser/lapaSelectionHelperWidget.js';
import { LAPA_OPEN_SETTINGS_ACTION_ID } from '../../../lapaSettingsPane.js';


export const LapaSelectionHelperMain = (props: LapaSelectionHelperProps) => {
	...
	>
		<LapaSelectionHelper {...props} />
	</div>
}



const LapaSelectionHelper = ({ rerenderKey }: LapaSelectionHelperProps) => {
	...
	const ctrlLKeybind = keybindingService.lookupKeybinding(LAPA_CTRL_L_ACTION_ID)
	const ctrlKKeybind = keybindingService.lookupKeybinding(LAPA_CTRL_K_ACTION_ID)
	...
	useEffect(() => {
		const disposable = commandService.onWillExecuteCommand(e => {
			if (e.commandId === LAPA_CTRL_L_ACTION_ID || e.commandId === LAPA_CTRL_K_ACTION_ID) {
	...
	const defaultHTML = <>
		{ctrlLKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(LAPA_CTRL_L_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
		...
		{ctrlKKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(LAPA_CTRL_K_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
	...
	const moreOptionsHTML = <>
		<div
			...
			'
			onClick={() => {
				commandService.executeCommand(LAPA_OPEN_SETTINGS_ACTION_ID);
				setClickState('clickedOption');
			}}
		>
			Disable Suggestions?
