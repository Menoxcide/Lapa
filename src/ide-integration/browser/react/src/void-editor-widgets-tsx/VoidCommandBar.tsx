...
import { LapaCommandBarProps } from '../../../lapaCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	LAPA_GOTO_NEXT_DIFF_ACTION_ID,
	LAPA_GOTO_PREV_DIFF_ACTION_ID,
	LAPA_GOTO_NEXT_URI_ACTION_ID,
	LAPA_GOTO_PREV_URI_ACTION_ID,
	LAPA_ACCEPT_FILE_ACTION_ID,
	LAPA_REJECT_FILE_ACTION_ID,
	LAPA_ACCEPT_ALL_DIFFS_ACTION_ID,
	LAPA_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const LapaCommandBarMain = ({ uri, editor }: LapaCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@void-scope ${isDark ? 'dark' : ''}`}
	>
		<LapaCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const LapaCommandBar = ({ uri, editor }: LapaCommandBarProps) => {
	...
	const lapaModelService = accessor.get('ILAPAModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(LAPA_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(LAPA_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(LAPA_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(LAPA_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(LAPA_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(LAPA_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(LAPA_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(LAPA_REJECT_ALL_DIFFS_ACTION_ID);
