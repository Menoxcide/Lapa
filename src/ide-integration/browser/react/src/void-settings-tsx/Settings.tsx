...
const SimpleModelSettingsDialog = ({
	...
	const settingsStateService = accessor.get('ILAPASettingsService')
	...
	const sourcecodeOverridesLink = `https://github.com/Menoxcide/Lapa/blob/main/lapa-ide-void/src/vs/workbench/contrib/lapa/common/modelCapabilities.ts`
...
const ProviderSetting = ({ providerName, settingName, subTextMd }: { providerName: ProviderName, settingName: SettingName, subTextMd: React.ReactNode }) => {
	...
	const handleChangeValue = useCallback((newVal: string) => {
		lapaSettingsService.setSettingOfProvider(providerName, settingName, newVal)
	}, [lapaSettingsService, providerName, settingName]);
...
// 	const needsModel = isProviderNameDisabled(providerName, lapaSettingsState) === 'addModel'

// 	// const accessor = useAccessor()
// 	// const lapaSettingsService = accessor.get('ILAPASettingsService')

// 	// const { enabled } = lapaSettingsState.settingsOfProvider[providerName]
...
// 				onChange={
// 					useCallback(() => {
// 						const enabledRef = lapaSettingsService.state.settingsOfProvider[providerName].enabled
// 						lapaSettingsService.setSettingOfProvider(providerName, 'enabled', !enabledRef)
// 					}, [lapaSettingsService, providerName])}
// 				size='sm+'
// 			/> */}
// 		</div>
...
export const AutoDetectLocalModelsToggle = () => {
	...
	const enabled = lapaSettingsState.globalSettings[settingName]
	...
	return <ButtonLeftTextRightOption
		leftButton={<VoidSwitch
			...
			value={enabled}
			onChange={(newVal) => {
				lapaSettingsService.setGlobalSetting(settingName, newVal)
...
export const AIInstructionsBox = () => {
	...
	return <VoidInputBox2
		...
		initValue={lapaSettingsState.globalSettings.aiInstructions}
		...
		onChangeText={(newText) => {
			lapaSettingsService.setGlobalSetting('aiInstructions', newText)
		}}
	/>
}
...
const FastApplyMethodDropdown = () => {
	...
	const onChangeOption = useCallback((newVal: boolean) => {
		lapaSettingsService.setGlobalSetting('enableFastApply', newVal)
	}, [lapaSettingsService])

	return <VoidCustomDropdownBox
		className='text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-1 rounded p-0.5 px-1'
		options={options}
		selectedOption={lapaSettingsService.state.globalSettings.enableFastApply}
...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	return <div
		...
		onClick={() => { lapaSettingsService.setGlobalSetting('isOnboardingComplete', false) }}
	>
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: { approvalType: ToolApprovalType, size: "xxs" | "xs" | "sm" | "sm+" | "md", desc: string }) => {
	...
	const onToggleAutoApprove = useCallback((approvalType: ToolApprovalType, newValue: boolean) => {
		lapaSettingsService.setGlobalSetting('autoApprove', {
			...lapaSettingsService.state.globalSettings.autoApprove,
			[approvalType]: newValue
		})
		metricsService.capture('Tool Auto-Accept Toggle', { enabled: newValue })
	}, [lapaSettingsService, metricsService])

	return <>
		<VoidSwitch
			size={size}
			value={lapaSettingsState.globalSettings.autoApprove[approvalType] ?? false}
...
const MCPServerComponent = ({ name, server }: { name: string, server: MCPServer }) => {
	...
	const lapaSettings = useSettingsState()
	const isOn = lapaSettings.mcpUserStateOfName[name]?.isOn
...
export const Settings = () => {
	...
	const onDownload = (t: 'Chats' | 'Settings') => {
		...
		if (t === 'Chats') {
			...
			dataStr = JSON.stringify(lapaSettingsService.state, null, 2)
		...
		reader.onload = () => {
			try {
				...
				if (t === 'Chats') {
					...
					lapaSettingsService.dangerousSetState(json as any)
				}
													...
													<ErrorBoundary>
														<div className='flex items-center gap-x-2 my-2'>
															<VoidSwitch
																...
																onChange={(newVal) => lapaSettingsService.setGlobalSetting('enableAutocomplete', newVal)}
															/>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => lapaSettingsService.setGlobalSetting('syncApplyToChat', newVal)}
														/>
												...
												<ErrorBoundary>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => lapaSettingsService.setGlobalSetting('includeToolLintErrors', newVal)}
														/>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => lapaSettingsService.setGlobalSetting('autoAcceptLLMChanges', newVal)}
														/>
												...
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => lapaSettingsService.setGlobalSetting('showInlineSuggestions', newVal)}
														/>
												...
												<div className='my-2'>
													...
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															...
															onChange={(newVal) => lapaSettingsService.setGlobalSetting('syncSCMToChat', newVal)}
														/>
								...
								<div>
									...
									<div className='flex flex-col gap-8'>
										...
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											...
											<ConfirmButton className='px-4 py-1 w-full' onConfirm={() => { lapaSettingsService.resetState(); }}>
												Reset Settings
											</ConfirmButton>
										</div>
										...
										<ErrorBoundary>
											<div className='flex items-center gap-x-2'>
												<VoidSwitch
													...
													onChange={(newValue) => {
														lapaSettingsService.setGlobalSetting('disableSystemMessage', newValue);
													}}
												/>
