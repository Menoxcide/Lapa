...
import { LAPA_ACCEPT_DIFF_ACTION_ID, LAPA_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { ILAPASettingsService } from '../common/lapaSettingsService.js';
import { FeatureName } from '../common/lapaSettingsTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@ILAPASettingsService private readonly _settingsService: ILAPASettingsService,
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._lapaModelService.initializeModel(model.uri)
	...
	// 		actions: {
	// 			secondary: [{
	// 				id: 'lapa.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open Void's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(LAPA_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/voideditor/lapa/issues/new) it.` : undefined
	// 	})
	// }
	...
	private _addDiffAreaStylesToURI = (uri: URI) => {
		const { model } = this._lapaModelService.getModel(uri)
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		...
		const onFinishEdit = async () => {
			...
			await this._lapaModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._lapaModelService.initializeModel(uri)
		await this._lapaModelService.saveModel(uri) // save the URI
	}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@ILAPACommandBarService private readonly _lapaCommandBarService: ILAPACommandBarService,
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(LAPA_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(LAPA_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._lapaCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._lapaCommandBarService.onDidChangeState(e => {
