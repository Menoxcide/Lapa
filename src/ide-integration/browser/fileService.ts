...
class FilePromptActionService extends Action2 {
	private static readonly LAPA_COPY_FILE_PROMPT_ID = 'lapa.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.LAPA_COPY_FILE_PROMPT_ID,
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await lapaModelService.getModelSafe(uri)).model?.getLanguageId() || '',
