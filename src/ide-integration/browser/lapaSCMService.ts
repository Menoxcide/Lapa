...
import { ILAPASCMService } from '../common/voidSCMTypes.js'
import { IMainProcessService } from '../../../../platform/ipc/common/mainProcessService.js'
import { ILAPASettingsService } from '../common/lapaSettingsService.js'
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js'
import { ILLMMessageService } from '../common/sendLLMMessageService.js'
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/lapaSettingsTypes.js'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private voidSCM: ILAPASCMService
	...
	constructor(
		...
		this.voidSCM = ProxyChannel.toService<ILAPASCMService>(mainProcessService.getChannel('void-channel-scm'))
	}
	...
	async generateCommitMessage() {
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const modelSelection = this.lapaSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.lapaSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.lapaSettingsService.state.overridesOfModel
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'lapa.generateCommitMessageAction',
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'lapa.loadingGenerateCommitMessageAction',
