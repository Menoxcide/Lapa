...
import { FetchFunction, loadSetting } from '@ai-sdk/provider-utils';
...
// Enhanced Local Agent with AI SDK compatibility
interface EnhancedLocalAgent extends LocalAgent {
  provider?: any;
  baseUrl?: string;
}

// Handoff evaluation from local Agent
...
export class LocalHandoffSystem {
  ...
  private async evaluateHandoff(context: Record<string, any>, task: Task): Promise<HandoffEvaluation> {
    ...
    try {
      ...
      if (evaluatorAgent.type === 'ollama') {
        evaluationResult = await sendOllamaChatRequest(
          evaluatorAgent.model,
          [{ role: 'user', content: `Evaluate this context and task for handoff: ${evaluationTask.input}` }]
        );
      } else if (evaluatorAgent.type === 'nim') {
        evaluationResult = await sendNIMInferenceRequest(
          evaluatorAgent.model,
          `Evaluate this context and task for handoff: ${evaluationTask.input}`
        );
      }
   ...
   */
  private async handoffToLocalAgent(
      ...
      if (targetAgent.type === 'ollama' && await isNIMAvailable()) {
        ...
        try {
          ...
          const ollamaResult = await this.runWithRetry(
            () => sendOllamaChatRequest(
              targetAgent.model,
              [{ role: 'user', content: `Process this task: ${JSON.stringify(context)}` }]
            ),
   ...
   * @returns Promise that resolves with the handoff result
   */
  async localHandoff(task: Task, context: Record<string, any>): Promise<any> {
    try {
 ...
 * @returns Promise that resolves with the handoff result
 */
export async function localHandoff(
  task: Task,
  context: Record<string, any>,
  preferredProvider: 'ollama' | 'nim' = 'ollama'
): Promise<any> {
  const startTime = performance.now();
