...
`;
     ...
     */
    private async initializeDebugServer(): Promise<void> {
        try {
...
`;
     ...
     */
    private async grepFile(pattern: string, path: string): Promise<string[]> {
        try {
     ...
     */
    private async findFiles(pattern: string, basePath: string = '.'): Promise<string[]> {
        try {
     ...
     */
    private async executeProcess(command: string, args: string[]): Promise<{ stdout: string; stderr: string; exitCode: number }> {
