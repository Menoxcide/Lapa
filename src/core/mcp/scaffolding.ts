...
// Type for templates
type TemplateType = 'function' | 'class';
type Templates = {
  typescript: Record<TemplateType, string>;
  javascript: Record<'function', string>;
  python: Record<'function', string>;
  shell: Record<'function', string>;
};
...
export class MCPScaffolding {
  ...
  private generateToolCode(config: MCPScaffoldingConfig): string {
    const template = TEMPLATES[config.language][config.templateType as keyof typeof TEMPLATES[typeof config.language]];
    if (!template) {
