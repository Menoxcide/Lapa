...
export class LicenseManager {
    ...
    private licenses: Map<string, LicenseData>;
    private config: any;
    ...
    constructor(secretKey?: string, config?: any) {
        ...
        this.secretKey = secretKey || process.env.LICENSE_SECRET_KEY || randomBytes(32).toString('hex');
    ...
    private validateInputs(userId: string, productId: string): void {
        ...
        if (userId.length === 36 && !validateUuid(userId)) {
    ...
    getUserLicenses(userId: string): LicenseData[] {
        try {
            const userLicenses: LicenseData[] = [];
     ...
     */
    private generateActivationKey(licenseId: string, userId: string, productId: string): string {
