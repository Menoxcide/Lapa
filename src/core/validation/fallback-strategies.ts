...
export class FallbackStrategiesManager {
   ...
   * Register default fallback providers
   */
  private registerDefaultFallbackProviders(): void {
    // Register local fallback provider for agent tools
    this.registerFallbackProvider('agent-tool-local', {
      canHandle: (operation: string) => operation.startsWith('agent-tool'),
    ...
    this.registerFallbackProvider('handoff-simplified', {
      canHandle: (operation: string) => operation === 'handoff',
      execute: async (operation: string, params: any) => {
   ...
   */
  registerFallbackProvider(name: string, provider: FallbackProvider): void {
   ...
   */
  private findSuitableFallbackProvider(operation: string): FallbackProvider | null {
   ...
   */
  private getProviderName(provider: FallbackProvider): string {
   ...
   */
  async gracefulDegradationForTool(
    tool: AgentTool,
    context: any
  ): Promise<any> {
   ...
   */
  async gracefulDegradationForModeSwitch(
    fromMode: RooMode,
    toMode: RooMode
  ): Promise<any> {
...
/**
 * Interface for fallback provider
 */
interface FallbackProvider {
  /**
   * Check if this provider can handle the specified operation
   * @param operation Name of the operation
   * @returns Boolean indicating if provider can handle the operation
   */
   ...
   * @param operation Name of the operation
   * @param params Parameters for the operation
   * @returns Promise that resolves with the fallback execution result
   */
  execute(operation: string, params: any): Promise<FallbackExecutionResult>;
}
