...
export class AdvancedVoiceAgent implements AdvancedVoiceAgentInterface {
  private audioPipeline: AudioProcessingPipeline;
  private eventBus: LAPAEventBus;
  private ragPipeline?: RAGPipeline;
  private config: VoiceAgentConfig;
  private isDictating: boolean;
  private dictationBuffer: Buffer[];
  private dictationStartTime: number;
  ...
  constructor(config?: VoiceAgentConfig, ragPipeline?: RAGPipeline) {
    ...
    this.audioPipeline = new TTSSTTPipeline(
      { provider: this.config.ttsProvider, modelPath: this.config.voiceModel },
      { provider: this.config.sttProvider, model: this.config.voiceModel, language: this.config.language }
    );
  ...
  async processAudio(audio: Buffer, format: string = 'wav'): Promise<SpeechToTextResult> {
    ...
    try {
      ...
      if (!await VoiceUtils.validateAudioBuffer(processedAudio)) {
        throw new Error('Invalid audio buffer');
      }
      
      // Convert to WAV if needed
      if (format.toLowerCase() !== 'wav') {
        processedAudio = await VoiceUtils.convertToWav(processedAudio, format);
      }
      
      // Remove silence
      processedAudio = await VoiceUtils.removeSilence(processedAudio);
  ...
  async generateAudio(text: string): Promise<TextToSpeechResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      const processingTime = Date.now() - startTime;
      const duration = VoiceUtils.estimateDuration(audioBuffer);
  ...
  async askQuestion(question: VoiceQuestion): Promise<VoiceAnswer> {
    const startTime = Date.now();
  ...
  async executeVoiceCommand(command: VoiceCommand): Promise<any> {
    ...
    try {
      ...
      const parsedCommand: ParsedCommand = command.intent && command.entities
        ? {
        ...
        : VoiceCommandParser.parseCommand(command.command);
      
      // Extract additional entities
      const additionalEntities = VoiceCommandParser.extractEntities(command.command);
   ...
   */
  async addToDictationBuffer(audio: Buffer): Promise<void> {
    if (this.isDictating) {
