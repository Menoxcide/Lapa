...
describe('AgentAvatars', () => {
  ...
  it('should display correct status indicators', () => {
    ...
    expect(statusIndicators[0]).toHaveClass('bg-blue-500'); // Working agent
    expect(statusIndicators[1]).toHaveClass('bg-gray-300'); // Idle agent
    expect(statusIndicators[2]).toHaveClass('bg-green-500'); // Completed agent
  });
  ...
  it('should display correct avatar initials and colors', () => {
    render(<AgentAvatars agents={[mockAgents[0]]} />);
    
    const avatarPlaceholder = screen.getByText('C'); // First letter of "Code Generator"
    ...
    expect(avatarDiv).toHaveStyle('background-color: rgb(255, 87, 51)'); // #FF5733 converted to RGB
  });
  ...
  it('should render with click handler and show hover effect', () => {
    const mockOnClick = vi.fn();
    render(<AgentAvatars agents={[mockAgents[0]]} onAgentClick={mockOnClick} />);
  ...
  it('should render without click handler and not show hover effect', () => {
    render(<AgentAvatars agents={[mockAgents[0]]} />);
