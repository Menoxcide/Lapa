...
describe('Swarm Workflow Integration', () => {
  ...
  describe('Context Handoff Between Agents', () => {
    it('should successfully handoff context between agents', async () => {
      ...
      // Complete handoff
      const receivedContext = await contextHandoff.completeHandoff(
        initiationResponse.handoffId,
        'agent-beta'
      );
  ...
  describe('Complete Swarm Workflow', () => {
    it('should execute a complete swarm intelligence workflow', async () => {
      ...
      const coderContext = await contextHandoff.completeHandoff(
      ...
      const reviewContext = await contextHandoff.completeHandoff(
        reviewHandoffResponse.handoffId,
        'reviewer-agent'
      );
      ...
      const votingSessionId = votingSystem.createVotingSession(
