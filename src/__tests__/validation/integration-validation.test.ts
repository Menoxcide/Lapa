  ...
  describe('End-to-End Validation Flow', () => {
    it('should validate, execute with recovery, preserve context, and track fidelity for a complete operation', async () => {
      ...
      setTimeout(() => {
        ...
        expect(rates.agentToolExecution).toBe(1); // Both attempts counted as one operation
        expect(rates.contextPreservation).toBe(1); // Successful preservation
      }, 10);
    });
    ...
    it('should handle cross-language communication with validation and error recovery', async () => {
      ...
      setTimeout(() => {
        ...
        expect(rates.crossLanguageCommunication).toBe(1);
      }, 10);
    });
  });
