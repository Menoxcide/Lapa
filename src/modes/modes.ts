...
import { ModeConfig, ModeState, ModeTransitionRequest, ModeTransitionResult, RooMode } from './types/mode-types.ts';
import { ModeGuard } from './types/mode-types.ts';
 ...
 */
export class RooModeController {
  private eventBus: LAPAEventBus;
  private modeState: ModeState;
  private modeConfigs: Map<RooMode, ModeConfig>;
  private modeGuards: ModeGuard[];
  private isInitialized: boolean;

  constructor(eventBusInstance: LAPAEventBus = eventBus) {
  ...
  async initialize(): Promise<void> {
    ...
    this.eventBus.subscribe('mode.change.request', async (event) => {
      const request = event.payload as ModeTransitionRequest;
   ...
   */
  getModeConfig(mode: RooMode): ModeConfig | undefined {
   ...
   * @returns Promise that resolves with the transition result
   */
  private async executeModeTransition(request: ModeTransitionRequest, startTime: number): Promise<ModeTransitionResult> {
   ...
   */
  updateModeConfig(mode: RooMode, config: ModeConfig): void {
...
export const rooModeController = new RooModeController();