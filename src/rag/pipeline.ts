...
import { optimizeChunksForLLM, shouldOptimizeForTOON } from '../utils/toon-optimizer.ts';
...
export class RAGPipeline {
  ...
  async processDocument(filePath: string): Promise<ProcessedDocument> {
    ...
    // Optimize chunks for LLM if suitable (large arrays benefit from TOON)
    let optimizedChunks: any = chunks;
    let chunksFormat: 'toon' | 'json' = 'json';
    let tokenReduction: number | undefined;

    if (shouldOptimizeForTOON(chunks, 10)) { // Optimize if 10+ chunks
      const optimizationResult = optimizeChunksForLLM(chunks);
      optimizedChunks = optimizationResult.optimized;
      chunksFormat = optimizationResult.format;
    ...
    const processedDoc = {
      ...
      metadata: {
        ...
        ...(chunksFormat === 'toon' && {
          chunksFormat: 'toon',
          chunksTokenReduction: tokenReduction
        })
      },
  ...
  async searchSimilar(
    ...
    try {
      ...
      if (this.refragEnabled && useREFRAG) {
        ...
        // Return expanded chunks (full content) + compressed chunks (embeddings only)
        // For backward compatibility, return expanded chunks as regular results
