// Voice command parser for the advanced voice agent
...
export class VoiceCommandParser {
  ...
  static parseCommand(command: string): ParsedCommand {
    ...
    const patterns = [
      ...
      },
      {
        ...
        intent: 'create_file',
        entities: (match: RegExpMatchArray) => ({
    ...
    for (const patternDef of patterns) {
      ...
      if (match) {
        ...
        const confidence = Math.min(
          patternDef.baseConfidence * (0.5 + 0.5 * matchRatio),
          0.95
        );
   ...
   */
  static extractEntities(command: string): Record<string, any> {
